...
<a href="https://evolink.ai/glm-5-2?utm_source=github&utm_medium=banner&utm_campaign=awesome-glm-5.2-usecases&utm_content=readme_banner"><img src="https://pub-62cf7640cd0f4066b60933bd2e9b85ef.r2.dev/github-repo-media/awesome-glm-5.2-usecases/images/en.jpg" alt="GLM-5.2 高訊號使用案例倉庫 banner" width="760"></a>
...
[在 Evolink 上試用 GLM-5.2](https://evolink.ai/glm-5-2?utm_source=github&utm_medium=readme&utm_campaign=awesome-glm-5.2-usecases&utm_content=introduction_cta)

## 📊 總覽

- **258 個精選 GLM-5.2 案例**，來自公開創作者、評測團隊、工具開發者、服務商與一線使用者。
...
[在 EvoLink 開啟 GLM-5.2](https://evolink.ai/glm-5-2?utm_source=github&utm_medium=quickstart&utm_campaign=awesome-glm-5.2-usecases&utm_content=model_link).

透過 Evolink 的 OpenAI 相容 Chat Completions API 使用 GLM-5.2。先在 [取得 API 金鑰](https://evolink.ai/dashboard/keys?utm_source=github&utm_medium=quickstart&utm_campaign=awesome-glm-5.2-usecases&utm_content=api_key) 取得 API key，然後在執行請求前設定為 `EVOLINK_API_KEY`。
...
閱讀完整 GLM-5.2 API 參考：[開啟 GLM-5.2 API docs](https://docs.evolink.ai/en/api-manual/language-series/glm-5.2/glm-5.2-api?utm_source=github&utm_medium=docs&utm_campaign=awesome-glm-5.2-usecases&utm_content=first_run).
...
| [💻 編碼代理與長上下文工作流](#coding-agents-long-context-workflows) | 案例 13-22, 62, 65, 66, 77, 80, 91, 102, 117, 119, 122, 127, 135-136, 142-143, 145, 148, 150, 153, 155, 168, 174, 180, 194, 210-212, 228, 236-237, 243, 255, 257 |
| [🎮 上手演示與作品展示](#hands-on-demos-showcase-builds) | 案例 23-30, 71, 78, 81-82, 92, 99-100, 123, 144, 158, 161, 192, 200, 202, 213, 218, 229 |
| [🔌 供應商與工具整合](#provider-tool-integrations) | 案例 31-42, 61, 63, 69, 74, 79, 83-87, 93, 95-96, 101, 104-105, 109, 115-116, 124-125, 128-130, 137, 141, 147, 152, 160, 165, 169-170, 176, 179, 185, 193, 195, 198, 201, 203-204, 208, 214, 219-220, 224-225, 230-232, 238-239, 256, 258 |
| [💸 成本、定價與本地部署](#cost-pricing-local-deployment) | 案例 43-51, 64, 68, 88-89, 97-98, 106-107, 112, 118, 131, 138-140, 151, 156, 164, 166, 171-173, 177, 181-183, 186-187, 191, 206, 209, 215, 221, 226, 233-234, 240-246, 249, 251, 253-254 |
...
| [Case 190: NatureBench 公開重量級亞軍](#case-190) | 如果你想看 GLM-5.2 在 scientific-agent 工作裡的基準表現，可以看這個案例，因為 NatureBench 說它在 6 個科學領域、90 個任務上首發即總榜第二，並拿到 open-weight 第一。 | Benchmark |
| [Case 189: 終端-工作台 45-任務成本權衡](#case-189) | 如果你想在同一套 agent harness 下比較 GLM-5.2 和 GPT-5.5，可以看這個案例，因為一組 45 個 Terminal-Bench 任務裡，GLM-5.2 解出 25 個，GPT-5.5 解出 29 個，但前者在 prompt caching 下便宜約 40%。 | Evaluation |
| [Case 188: Harvey LAB-AA 法律代理領帶](#case-188) | 如果你想看 GLM-5.2 在真實法律 agent 工作裡的基準表現，可以看這個案例，因為 Harvey LAB-AA 顯示 GLM-5.2 Max 在 24 個法律領域、120 個私有任務上拿到 7.5% all-pass，並列 Claude Opus 4.8。 | Benchmark |
| [Case 184: AutomationBench-AA 開放式重量領先](#case-184) | 如果你想比較 GLM-5.2 在必須遵守 business rule 的 SaaS automation 裡的表現，而不只是看 coding benchmark，可以看這個案例，因為 Artificial Analysis 報告 GLM-5.2 Max 在 AutomationBench-AA 上拿到 27.8%，並稱它是 open weights 裡的第一名。 | Evaluation |
| [Case 178: 三體模擬器基準獲勝](#case-178) | 如果你想在帶數值物理約束的 coding benchmark 裡比較 GLM-5.2，可以看這個案例，因為 AlicanKiraz0 跑了一個混沌三體模擬器任務，並給 GLM 5.2 Max 打出 91/100 的最高分。 | Evaluation |
| [Case 167: GameDevBench 333 任務開源負責人](#case-167) | 如果你想在 agentic 遊戲開發 benchmark 裡追蹤 GLM-5.2，可以看這個案例，因為 GameDevBench 已擴充到 333 個任務，並表示即使沒有 vision，GLM-5.2 仍是排行榜上最強的 open-source model。 | Evaluation |
| [Case 175: 遊標雙擺記分卡](#case-175) | 如果你想在一個受限的 Cursor coding benchmark 裡比較 GLM-5.2，可以看這個案例，因為 AlicanKiraz0 用 HTML double-pendulum simulator 跑了 6 個模型，給 GLM 5.2 Max 打了 88/100，雖然落後 Fable 和 Sonnet，但仍高於 GPT-5.5、Kimi K2.7 Code 和 Composer。 | Evaluation |
| [Case 162: VulcanBench 10 項任務 80% 平局](#case-162) | 如果你想在成本和分數同樣重要的 post-cutoff 真實工程任務中比較 GLM-5.2，可以看這個案例，因為 Morgan Linton 說 VulcanBench 讓 GLM 5.2 High、Fable 5 Low 和 Sonnet 5 High 在 10 個 repo 上都拿到 80%，而 GLM 的成本落在中間。 | Evaluation |
| [Case 159: SWE-重新基準 51.1% 檢查點](#case-159) | 如果你想持續跟蹤 GLM-5.2 在 SWE agent 榜單上的位置，可以看這個案例，因為最新一條 SWE rebench 貼文給出的成績是 51.1%，消耗 262 萬 token，明顯高於新加入的 DeepSeek、MiMo、Qwen 和 Gemma。 | Evaluation |
| [Case 154: 推出 Darkly Edge-Case 以 40/41 獲勝](#case-154) | 如果你想看 GLM-5.2 在企業工具型 agent 任務裡的表現，而不是只看聊天評測，可以用這個案例，因為 Composio 說它在 GitHub、Jira 和 LaunchDarkly 的 41 個任務裡做對了 40 個，而且只有 GLM 抓到了一个待審批邊界條件。 | Evaluation |
| [Case 146: CyberBench 開放重量補丁亞軍](#case-146) | 如果你想衡量 GLM-5.2 在偏攻防式漏洞發現與補丁生成上的能力，可以用這個案例，因為 CyberBench 把它放在 60 個真實 OSS-Fuzz 漏洞上的總榜第二。 | Evaluation |
| [Case 1: 人工智慧分析智能指數](#case-1) | 使用人工分析貼文將 GLM-5.2 與其他開放權重和專有前沿模型在智慧和每項任務成本方面進行比較。 | Benchmark |
| [Case 2: Code Arena 前端排名](#case-2) | 使用此案例在透過競技場式比較判斷的真實前端編碼任務上評估 GLM-5.2。 | Benchmark |
| [Case 3: 設計競技場第一名](#case-3) | 使用此案例來判斷 GLM-5.2 是否可以處理設計加代碼任務，而不僅僅是文字密集型編碼基準。 | Benchmark |
| [Case 4: FrontierSWE 結果](#case-4) | 使用 FrontierSWE 貼文將 GLM-5.2 與 GPT-5.5、Opus 和 Fable 風格的模型在軟體工程任務上進行比較。 | Benchmark |
| [Case 5: DeepSWE 開源負責人](#case-5) | 使用 DeepSWE 案例了解 GLM-5.2 作為用於困難的軟體工程評估任務的強大開放模型。 | Benchmark |
| [Case 6: 終端工作台超過 80%](#case-6) | 在評估面向終端的編碼和代理工作流程的 GLM-5.2 時使用此案例。 | Benchmark |
| [Case 7: SWELancer 與 GPT-5.5 的比較](#case-7) | 使用此 SWELancer 案例作為 GLM-5.2 和 GPT-5.5 在任務成功、獎勵和完成時間方面的具體多指標比較。 | Evaluation |
| [Case 8: BridgeBench 滿分訊號](#case-8) | 使用此案例來檢查基於多步驟推理的 GLM-5.2，而不僅僅是編碼排行榜。 | Benchmark |
| [Case 9: BridgeBench 推理排名第一](#case-9) | 使用此案例在紮根推理任務上將 GLM-5.2 與封閉邊界模型進行比較。 | Benchmark |
| [Case 10: KernelBench-Hard 沒有捷徑](#case-10) | 在檢查基準測試收益是否來自有效的實現行為而不是捷徑時使用此案例。 | Evaluation |
| [Case 11: Runescape 長椅追趕](#case-11) | 使用此案例作為開放權重模型在類似遊戲的基準任務上取得進展的快速訊號。 | Benchmark |
| [Case 12: BridgeBench 速度提升](#case-12) | 使用此案例來評估對延遲敏感的工作流程，其中速度與智慧同樣重要。 | Benchmark |
| [Case 60: KernelBench 硬核與巨型 GPU 編碼](#case-60) | 使用此案例跨 KernelBench-Hard 和 KernelBench-Mega 評估 GPU 核心編碼上的 GLM-5.2，其中開放代理追蹤使結果可檢查。 | Benchmark |
...
| [Case 90: GDPval-AA 代理工作指數](#case-90) | 如果你想比較 GLM-5.2 在長期知識工作中的表現，而不只是看編碼榜單，可以用這個案例。 | Evaluation |
| [Case 94: 遊戲開發競技場亞軍](#case-94) | 如果你想判斷 GLM-5.2 的遊戲建構品質，可以用這個案例。它在 Game Dev Arena 拿到第二名，並成為該榜單中開放權重陣營的第一名。 | Evaluation |
| [Case 120: PostTrainBench 可靠性領先](#case-120) | 如果你想比較 GLM-5.2 Max，不只看 headline 分數，也看它在 84 個任務上 failed run 為 0 的 agent reliability，可以用這個案例。 | Benchmark |
| [Case 121: Fireworks + Faros 211 項儲存庫任務評測](#case-121) | 如果你想在 private repo 的真實 engineering task 上評估 GLM-5.2，而不是只看公開 benchmark，可以用這個案例；貼文同時給出了分數、速度與每任務成本。 | Evaluation |
| [Case 110: AA-Briefcase 每任務耗時前沿](#case-110) | 用這個案例比較 GLM-5.2 在長時程知識工作上的表現，因為除了 benchmark 分數，單任務耗時同樣重要。 | Benchmark |
| [Case 111: Code Arena 前端對戰優勢](#case-111) | 用這個案例查看 GLM-5.2 在前端任務上的成對對戰優勢，而不是只看一張排名截圖。 | Benchmark |
| [Case 113: SWE Atlas 程式庫問答亞軍](#case-113) | 用這個案例追蹤 GLM-5.2 在程式庫問答、測試撰寫與重構三條 SWE Atlas 榜單上的表現，而不是只看單項 SWE 榜單。 | Benchmark |
...
| [Case 228: OpenCode 本地 agentic coding 底座](#case-228) | 如果你想在付費訂閱 frontier 模型前先驗證本地 coding-agent stack，可以看這個案例，因為 comma_ai 說他們已經移除 Anthropic，並發現 GLM 5.2 加 OpenCode 的 agentic coding 流程更好用。 | Demo |
| [Case 212: Dell Hub GLM Agent 教學](#case-212) | 如果你想為開放權重訓練工作流搭一套 GLM-5.2 coding agent，可以看這個案例，因為 juanjucm 表示，一篇新指南把 Dell Enterprise Hub 新增 GLM-5.2-FP8 目錄更新，與一套圍繞該模型構建 agent 的逐步搭建流程放在了一起。 | Tutorial |
| [Case 211: 8xB200 開放權重報告流水線](#case-211) | 如果你想把 GLM-5.2 作為主筆，接進一條貼近本地部署的報告流水線，可以看這個案例，因為 TheZachMueller 表示，一台 8xB200 節點按 4/4 切分後，可以讓 GLM 5.2 NVFP4 負責報告生成、Kimi K2.7 Code 負責檢索，以相對 Claude API 幾乎只是零頭的成本產出一份更紮實的 36 頁報告。 | Demo |
| [Case 210: Spettro 的 Liquid Glass 多代理改版](#case-210) | 如果你想把 GLM-5.2 當成一個研究密集型 frontend fixer，放進多代理網站改版流程裡測試，可以看這個案例，因為 spettrotoken 表示在 Fable 5 和 GPT-5.5 都失敗之後，GLM 5.2 透過整合好的 web scraping 與 data fetching 工具，做出了可在 Firefox 運作的跨瀏覽器 Liquid Glass 實作。 | Demo |
| [Case 194: CuTeDSL 內核技能開源](#case-194) | 如果你想研究 GLM-5.2 在可重用 kernel 除錯 skill 裡的表現，可以看這個案例，因為作者把一個 CuTeDSL 的 Hermes skill 開源出來，並明確說 GLM-5.2 在除錯和編寫 kernels 時特別省成本。 | Tutorial |
| [Case 180: Hermes SSD 恢復技能循環](#case-180) | 如果你想在面向修復的 agent loop 裡測試 GLM-5.2，可以看這個案例，因為 ShankhadeepSho1 說 Hermes 加 GLM 5.2 診斷了一塊故障 NAS SSD，修好了問題，然後把修復方案封裝成可重用 skill。 | Demo |
| [Case 174: 角色路由的重型編碼器堆疊](#case-174) | 如果你想把 GLM-5.2 放進一套按角色路由的個人 stack，專門承擔較重的 coding 工作，可以看這個案例，因為 denizirgin 表示，在用 Codex 和 OpenCode 測了一個月之後，他把更重的 coding work 交給了 GLM 5.2，同時把總月預算控制在 120 到 140 美元左右。 | Evaluation |
| [Case 155: Cotal 四代理 TUI 循環](#case-155) | 如果你想把一個編碼循環拆給不同專長的 agent，可以看這個案例，因為作者用兩個 GLM-5.2 worker，外加一個 Opus 負責人和一個 GPT reviewer，在 47 分鐘內無人干預地做完了一個 lazygit 風格的完整 TUI。 | Demo |
| [Case 153: 舊版遷移成本削減試點](#case-153) | 如果你想評估 GLM-5.2 在遺留應用改造流程裡能不能當更便宜的執行模型，可以看這個案例，因為 8090 的試點說 GLM 加 Software Factory 相比單跑 Opus 4.8 把成本壓到了 1/16.4，但速度大約慢了 3 倍。 | Evaluation |
| [Case 150: Mac Studio瀏覽器-使用本機循環](#case-150) | 如果你想測試一套完全本地的 GLM-5.2 棧，是否已能在消費級硬體上完成輕量 browser agent 工作，可以用這個案例，因為作者在 Mac Studio 上用 llama.cpp 和 browser-use 到 Hugging Face 找到了一個 PII 模型。 | Demo |
| [Case 148: Gumloop 代理交換成本削減](#case-148) | 如果你想在現有 agent harness 裡直接試一次模型替換，可以用這個案例，因為 Gumloop 表示把最常用的 agent 換成 GLM-5.2 後，使用者幾乎沒感到品質下降，而 credits 消耗大約少了 50%。 | Evaluation |
| [Case 13: 一小時四十二分鐘重構循環](#case-13) | 使用此案例作為使用 TDD、審閱者回饋和回歸檢查進行長時間自主前端重構的模式。 | Demo |
| [Case 14: OpenCode Bug 修復與實作測試](#case-14) | 使用此案例來測試 GLM-5.2 作為 OpenCode 編碼代理程式的錯誤修復以及小型實作任務。 | Demo |
| [Case 15: OpenCode 復古電玩演練](#case-15) | 使用此演練透過單一提示使用 GLM-5.2 和 OpenCode 建立一個小遊戲，然後檢查模型如何處理實作細節。 | Tutorial |
| [Case 16: HTML5 實體模擬競賽](#case-16) | 使用此案例在沒有庫的獨立 HTML5 物理模擬上比較 GLM-5.2 和 Kimi K2.7 程式碼。 | Evaluation |
| [Case 17: 個人網站 UI UX 構建](#case-17) | 使用此案例提示 GLM-5.2 打造一個精美的個人網站，並檢查多次轉動可以在多大程度上改善 UI/UX。 | Demo |
| [Case 18: AI合約審查產品構建](#case-18) | 使用此案例透過 PRD、時間預算、步驟計數、使用配額和程式碼品質比較來評估產品建置任務上的 GLM-5.2。 | Evaluation |
| [Case 19: ZCode 目標功能可實現更大的開發目標](#case-19) | 使用此案例了解如何將 GLM-5.2 整合到 ZCode 3.0 中以執行更大的代理開發任務。 | Integration |
| [Case 20: 使用 GLM-5.2 建構的 ZCode 的 Linux 包裝器](#case-20) | 使用此案例作為 GLM-5.2 協助編碼代理環境工具的範例。 | Demo |
| [Case 21: 計算機使用技能包裝](#case-21) | 使用此案例來研究 GLM-5.2，將其作為將開源電腦使用儲存庫轉變為可重複使用技能的助手。 | Demo |
| [Case 22: ZCode 3.0代理開發環境審查](#case-22) | 使用此案例在完整的代理開發環境而不是單一聊天會話中評估 GLM-5.2。 | Demo |
| [Case 62: 具有本地服務的 OpenCode Harness](#case-62) | 使用此案例透過 OpenCode 工具、本機服務和工具密集型編碼工作流程來測試 GLM-5.2，然後將其與 Claude Opus 進行比較。 | Evaluation |
| [Case 65: Fast-RLM 長上下文指令注入](#case-65) | 使用此案例透過將指令移至 RLM 系統提示字元來改進 GLM-5.2 長上下文計數和 REPL 代理行為。 | Integration |
| [Case 66: DeepAgents 程式碼開放式線束試用](#case-66) | 使用此案例嘗試使用開放式編碼代理工具的 GLM-5.2，並在可重現的代理 shell 下比較模型。 | Demo |
| [Case 77: 生產級行銷 Agent 棧路由策略](#case-77) | 用這個案例把 GLM-5.2 路由到重視結構化、速度與自託管的生產 Agent 工作流中，同時把更強的閉源模型留給模糊判斷任務。 | Evaluation |
| [Case 80: M3 究極精靈寶可夢 紅色目標跑](#case-80) | 用這個案例評估 GLM-5.2 在長時間本地 coding agent 執行中的表現，追蹤它在 M3 Ultra 上花接近半天重建 Pokemon Red HTML 版本的嘗試。 | Demo |
| [Case 91: Cline Repo 錯誤修復攤牌](#case-91) | 如果你想比較 GLM-5.2 與 Opus 4.8 在真實儲存庫 bug 修復中的表現，可以用這個案例。GLM 雖然用了更多 token，但最後產出的 patch 更便宜也更乾淨。 | Evaluation |
| [Case 102: OpenInspect FP8 後台代理](#case-102) | 如果你想研究自託管的 GLM-5.2 背景 agent 棧，而不是託管聊天工作流，可以用這個案例。 | Integration |
| [Case 145: OpenCode + Fireworks 降本遷移](#case-145) | 如果你想測試只換 open-model harness 是否已經足夠，可以用這個案例，因為作者把個人 coding 與 loop task 遷到 Fireworks 上的 GLM-5.2 + OpenCode 後，稱 token 成本降到了三分之一，而且日常品質沒有明顯下滑。 | Evaluation |
| [Case 143: Hermes MoA 的 GLM 聚合器工作流](#case-143) | 如果你願意為高價值 agent 回合多做一層編排，可以用這個案例，因為 Hermes Agent 的 MoA 設定把 GLM-5.2 和其他模型組合後，在貼文裡的 demo 中用很小的增量成本換來更好的輸出。 | Integration |
| [Case 142: Cline 推理開關帶來的 Harness 差值](#case-142) | 如果你想判斷決定結果的是 harness 還是權重本身，可以用這個案例，因為同一個 GLM-5.2 在同一批 coding task 上，只是打開 reasoning，結果就從 57.3% 跳到 68.5%。 | Evaluation |
| [Case 136: Cursor + Fireworks 4.55 億 Token 實戰測試](#case-136) | 如果你想判斷 GLM-5.2 能否成為 Cursor 的日常主力模型，可以用這個案例，因為作者回報在 Fireworks 服務下真實跑了 4.55 億 tokens，且暫時不想回到 Opus 或 GPT-5.5。 | Evaluation |
| [Case 135: Devin Desktop Harness 與 Skill 可攜性](#case-135) | 如果你覺得 Z.ai 自家的 coding 介面不夠穩定，想測試 GLM-5.2 在 Devin Desktop 裡的表現，可以用這個案例，因為作者回報 skill 遷移更容易、速度更快，也更好 hack。 | Evaluation |
| [Case 127: Pi 內聯 GLM 審閱者](#case-127) | 如果你想在 Pi 風格的 coding-agent loop 裡加入第二位審閱者，可以用這個案例。作者表示，GLM-5.2 可以逐回合給 Opus 提建議，成本增幅大約只有 10%。 | Integration |
| [Case 122: AgentRouter 一次成型 Telegram Bot](#case-122) | 如果你想測試 GLM-5.2 在 one-shot coding-agent build 裡，是否能主動補上偏向生產環境的 defaults，而不是只寫出最低限度可運作的路徑，可以用這個案例。 | Demo |
| [Case 117: OpenCode Go 重構首輪取勝](#case-117) | 用這個案例評估 GLM-5.2 在 OpenCode 中處理中等規模 Go 重構任務的表現，而不是只看 benchmark 宣傳。 | Evaluation |
| [Case 119: Claude Code + Cursor 預設執行成本 3.36 美元](#case-119) | 用這個案例判斷 GLM-5.2 是否適合作為 Claude Code 與 Cursor 的日常預設模型，再決定是否把更多自主編碼工作遷移到開放權重模型上。 | Evaluation |

### [🎮 上手演示與作品展示](#hands-on-demos-showcase-builds)

| 案例 | 展示重點 | 類型 |
|---|---|---|
| [Case 229: Hyperagent 個人首頁作品集對決](#case-229) | 如果你想在真實 browser-based agent 任務裡比較 GLM-5.2 和其他 open model，可以看這個案例，因為 arsh_goyal 把 GLM 5.2、DeepSeek V4、Kimi K2.6 和 Qwen 3.7 並排放到 Hyperagent 上，用公開資料自動生成個人作品集。 | Demo |
...
| [Case 192: 命令代碼 Flappy Bird UX Split](#case-192) | 如果你想看 GLM-5.2 在輕量設計類小遊戲任務裡的性價比，可以看這個案例，因為作者用同一個 Flappy Bird prompt 跑了 Command Code，最後認為 Fable 5 雖然價格接近 GLM-5.2 的 9 倍，但在 UX 上並沒有明顯更好。 | Evaluation |
| [Case 161: REAP NVFP4 魔術方塊一擊](#case-161) | 如果你想測試 GLM-5.2 在單一 prompt 的互動式 build 任務上的表現，可以看這個案例，因為 REAP-NVFP4 的 demo 說它只靠一個 prompt 就做出了 3D Rubiks Cube、真實 scramble、即時狀態和 solve 按鈕。 | Demo |
| [Case 158: OMP 中繼 iPhone 用戶端](#case-158) | 如果你想把一個本地 GLM-5.2 agent 很快包進移動端介面，可以看這個案例，因為作者說 Codex 的 build-ios-app plugin 只用了幾個小時，就給一個已經接了 GLM-5.2 和 Cloudflare 隧道的 OMP relay 做出了成品感很強的 iPhone 客戶端。 | Demo |
| [Case 144: 開源 DevRel 研究 Agent](#case-144) | 如果你想把 GLM-5.2 從通用聊天模型變成垂直研究助手，可以用這個案例，因為作者做了一個開源 DevRel agent，能把產品和受眾輸入轉成帶證據與提綱的選題列表。 | Demo |
| [Case 123: Recast 六版本落地頁迭代流程](#case-123) | 如果你想先用 GLM-5.2 產出多個版本，再把最佳結果交給 coding agent 繼續做，以低成本試作 landing page，可以用這個案例。 | Tutorial |
| [Case 23: 可玩的密室一擊](#case-23) | 使用此案例來比較 GLM-5.2 和 Opus 4.8 之間相同提示的遊戲建立輸出、運行時間和成本。 | Demo |
| [Case 24: 三個真實的構建結果參差不齊](#case-24) | 將此案例用作警示演示集：在信任用於生產遊戲或視訊任務的模型之前測試多個真實構建。 | Evaluation |
| [Case 25: ZCode 中的超級瑪利歐克隆](#case-25) | 使用此案例來評估使用 GLM-5.2 和 ZCode 在多個修復和功能過程中進行的迭代遊戲建置。 | Demo |
| [Case 26: 月球登陸器競賽](#case-26) | 使用此案例在互動式遊戲類型任務上比較 GLM-5.2、MiniMax M3 和 Kimi K2.7 代碼。 | Evaluation |
| [Case 27: 一鍵設計競技場創建](#case-27) | 使用此案例來檢查 GLM-5.2 可以從競技場上下文中的單一設計提示產生什麼。 | Demo |
| [Case 28: 研究論文理解工作流程](#case-28) | 使用此案例將 GLM-5.2 應用於包含上下文問題和跨論文參考的論文閱讀工作流程。 | Integration |
| [Case 29: 受限制的詩歌比較](#case-29) | 在將 GLM-5.2 與寓言風格模型進行比較時，使用此案例將正確性與創意品質分開。 | Evaluation |
| [Case 30: 設計感範例](#case-30) | 使用此案例作為輕量級視覺設計訊號，然後使用您自己的提示和 UI 審查進行驗證。 | Demo |
...
| [Case 81: 太空侵略者一鍵構建](#case-81) | 用這個案例測試 GLM-5.2 的單提示詞遊戲生成能力，並觀察少量後續回合如何完成素材替換與輕量打磨。 | Demo |
| [Case 82: OpenCode 復原實驗室一次性](#case-82) | 用這個案例快速原型化互動式 agent failure simulation；作者回報約 3.50 美元就一輪做出了可運作的 recovery lab。 | Demo |
| [Case 92: 開啟設計參考 URL 重建](#case-92) | 如果你想測試 GLM-5.2 在參考驅動網站重建上的能力，可以用這個案例。只給一個來源 URL 和一條提示詞，就幾乎像素級重現了網站。 | Demo |
| [Case 99: 交易台成本品質測試](#case-99) | 如果你想比較 GLM-5.2 在全端 UI 建構上的表現，可以用這個案例。它做出的交易桌面結果接近最精緻的版本，但成本只占頂尖結果的一小部分。 | Evaluation |
| [Case 100: 克勞德拒絕後的勒德分子遊戲](#case-100) | 如果閉源模型因政策原因拒絕請求，而你又想原型化帶政策敏感性的遊戲概念，可以用這個案例測試 GLM-5.2。 | Demo |

### [🔌 供應商與工具整合](#provider-tool-integrations)

| 案例 | 展示重點 | 類型 |
|---|---|---|
| [Case 239: TokenRouter 免費 GLM API 視窗](#case-239) | 如果你想快速拿到一條短期免費的 GLM-5.2 API 路徑，可以看這個案例，因為 meliasiih 說 TokenRouter 提供到 2026-07-25 為止的免費存取，並且流程包含簡單註冊、API key 與公開 base URL。 | Tutorial |
| [Case 238: Agentuity Wafer GLM 閘道](#case-238) | 如果你想把 GLM-5.2 接進 Agentuity gateway stack，可以看這個案例，因為 wafer_ai 說它的 serverless 路線現在已在 Agentuity 上提供 GLM 5.2，regular 和 Fast tier 都支援約 100 到 250 tok/s 與 1M context。 | Integration |
| [Case 232: Macroscope Check Run GLM 代理](#case-232) | 如果你想在保留 check-run workflow 的同時壓低 PR review agent 成本，可以看這個案例，因為 kayvz 說 Macroscope 的 Check Run Agents 現在可以直接從一般 `.md` repo 設定裡選用 GLM 5.2。 | Integration |
| [Case 231: Aster 281 TPS 研究代理 API](#case-231) | 如果你想 benchmark 一個高速 hosted GLM-5.2 endpoint，可以看這個案例，因為 asterailabs 說 Aster Inference 把來自 research-agent 優化的 API 路由以 281 tokens per second 提供給 GLM 5.2。 | Integration |
| [Case 230: TrueFoundry 原生 Wafer GLM 路由](#case-230) | 如果你想把 GLM-5.2 接進現有 TrueFoundry AI Gateway stack，可以看這個案例，因為 wafer_ai 說這條 native provider integration 從 GLM 5.2 和 GLM 5.2 Fast 開始，而且不需要改動閘道其餘部分。 | Integration |
| [Case 225: TogetherLink Codex Harness 橋接](#case-225) | 如果你想把 GLM-5.2 跑進現有 coding-agent CLI，可以看這個案例，因為 nutlope 說 TogetherLink 是一個開源 CLI，能讓 Codex 和 Claude Code 直接呼叫 GLM 5.2 這類 open model。 | Integration |
| [Case 224: Vorflux 開放模型 Harness 路由](#case-224) | 如果你想在不寫自訂 glue code 的前提下，把 GLM-5.2 接進完整 agent session，可以看這個案例，因為 vorfluxai 說它的 Open Model Harness 會把 GLM 5.2 分配給 design、build 和 simplify 步驟，同時保留 Vorflux 其餘流程不變。 | Integration |
...
| [Case 198: Dahl 100M 免費 GLM 端點](#case-198) | 如果你想走一條不用綁卡、又相容 OpenAI 的路徑來試 GLM-5.2，可以看這個案例，因為 Dahl Inference 給 GLM 5.2 FP8 提供了 1 億免費 tokens，並且寫清了如何建 key、如何呼叫 `/v1/chat/completions`。 | Tutorial |
| [Case 195: NVIDIA 免費端點 GLM 設定](#case-195) | 如果你想在不 self-hosting 的情況下把 GLM-5.2 接進 coding tools 裡測試，可以看這個案例，因為原貼文給出了一條免費的 NVIDIA endpoint 路線，直接把 GLM-5.2 的 API key 用到 Claude Code、Cursor 或 Cline。 | Tutorial |
| [Case 193: 0G TeeML 私有推理路由](#case-193) | 如果你想把 GLM-5.2 接到一條更重視隱私的 provider 路線上，可以看這個案例，因為 0G 說 GLM 5.2 已經跑在 TeeML 上，prompts 被封裝在 TEE enclave 裡，而且價格低於官方路線。 | Integration |
| [Case 185: DuckDB Flock 的 Open-SQL PR](#case-185) | 如果你想把 GLM-5.2 帶進完全開源的本地 SQL analysis，可以看這個案例，因為 lhoestq 說，一個 duckdb + flock 的 PR 已經讓 GLM-5.2 進入 100% open-source 的資料堆疊。 | Integration |
| [Case 179: 一鍵26款API接入](#case-179) | 如果你想先透過單一 OpenAI 相容 provider 試用 GLM-5.2，可以看這個案例，因為 Alan_Earn 說一把 API key 就能存取 GLM-5.2 和另外 25 個模型，還附帶 26 美元起始 credits。 | Tutorial |
| [Case 176: NVIDIA NIM OpenCode 思維設置](#case-176) | 如果你想透過 NVIDIA 免費 NIM endpoint 把 GLM-5.2 接進 OpenCode，並且走一條明確開啟 thinking 的零成本路線，可以看這個案例，因為 Dracoshowumore 分享了 API key 流程、base URL，以及一份由工具層接管 function calls、讓 GLM 以 enable_thinking=true 運行的 OpenCode 設定。 | Tutorial |
| [Case 165: 透過行動代理控制啟動 ZCode](#case-165) | 如果你想把 ZCode 當成 GLM-5.2 的官方 coding surface 來評估，可以看這個案例，因為 launch report 說這個免費的 agentic IDE 會上 Windows、macOS、Linux，還能透過 Telegram、WeChat、Feishu 追蹤專案進度。 | Integration |
| [Case 160: OpenWiki 自動維護代理文檔](#case-160) | 如果你想讓 agent 可讀的文件自動保持最新，可以看這個案例，因為 LangChain 說 OpenWiki 會隨著程式碼變動重建並維護 repo docs，而且能跑在 GLM 5.2 這類開源模型上。 | Integration |
| [Case 152: 透過 FireConnect 鑄造 PTU](#case-152) | 如果你想在不重寫 agent 用戶端的前提下，把 GLM-5.2 接到企業級 Foundry 預算裡，可以用這個案例，因為 Fireworks 說 FireConnect 能把 Microsoft Foundry 的 PTU 接到 Codex、OpenCode 和 Pi 工作流中。 | Integration |
| [Case 147: Braintrust GLM 評估工作台](#case-147) | 如果你想把 GLM-5.2 和 Opus 放進同一套 eval 棧裡比較，可以用這個案例，因為 Braintrust 聯合 Baseten 給出了一個帶長上下文成本與精度對照的接入樣例。 | Integration |
| [Case 141: ClinePass 開放權重模型統一訂閱](#case-141) | 如果你想把多個開放權重 coding model 收攏到同一個 agent harness 裡，可以用這個案例，因為 ClinePass 把 GLM-5.2 和相關模型打包成統一月費，而不是分別維護 provider key 和帳單。 | Integration |
| [Case 137: 供 Coding Agents 使用的免費 GLM API 服務](#case-137) | 如果你想在不註冊的情況下，把 GLM-5.2 接入 Hermes 或其他 coding agent，可以用這個案例，因為這個共享服務會發放短時效 API key，整體設定相當輕量。 | Integration |
| [Case 31: OpenCode Go 可用性](#case-31) | 使用此案例透過文字、1M 上下文和類似 GLM-5.1 的定價來追蹤 OpenCode Go 工作流程中的 GLM-5.2 可用性。 | Integration |
| [Case 32: 奧拉馬雲端可用性](#case-32) | 使用此案例將 GLM-5.2 路由到 Ollama Cloud 中，以進行可存取的開源編碼模型實驗。 | Integration |
| [Case 33: OpenRouter One API 呼叫存取](#case-33) | 在比較提供者路由或多模型堆疊時，使用此案例透過 OpenRouter 存取 GLM-5.2。 | Integration |
| [Case 34: vLLM 零日支持](#case-34) | 使用此案例透過 vLLM 自行託管或提供 GLM-5.2 服務，並提供零日支援。 | Integration |
| [Case 35: 透過 Baseten 獲得概念可用性](#case-35) | 使用此案例將 GLM-5.2 識別為 Notion 工作流程中可用的開放權重模型。 | Integration |
| [Case 36: 煙火零日服務](#case-36) | 使用此案例來評估 Fireworks 作為需要託管基礎架構的 GLM-5.2 工作負載的服務路徑。 | Integration |
| [Case 37: 谷歌雲模型花園鏈接](#case-37) | 使用此案例在 Google Cloud 導向的部署和代理平台上下文中尋找 GLM-5.2。 | Integration |
| [Case 38: 威尼斯隱私模式](#case-38) | 當隱私模式、TEE 或端對端加密是嘗試 GLM-5.2 的決定因素時，請使用此案例。 | Integration |
| [Case 39: 命令代碼可用性](#case-39) | 使用此案例嘗試命令代碼中的 GLM-5.2，具有低成本入門計劃和長上下文編碼功能。 | Integration |
| [Case 40: 來自 Nous Portal 的赫爾墨斯特工](#case-40) | 使用此案例透過 Nous Portal 和 OpenRouter 將 GLM-5.2 連接到 Hermes Agent 工作流程。 | Integration |
| [Case 41: io.net 零日啟動合作夥伴](#case-41) | 在評估 753B 參數長上下文模型的計算支援服務時使用此案例。 | Integration |
| [Case 42: 模組化雲端零日服務](#case-42) | 使用此案例來考慮在提供者規模提供長上下文 GLM-5.2 服務的模組化雲端。 | Integration |
| [Case 61: 透過 OpenRouter 設定遊標](#case-61) | 使用此案例透過 OpenRouter 在 Cursor 中設定 GLM-5.2，以實現低成本的開放模型編碼工作流程。 | Tutorial |
| [Case 63: 放大視覺設計的代理眼睛](#case-63) | 當純文字模型需要視覺審核支援來完成設計任務時，可以使用此案例將 GLM-5.2 與 Amp 自訂代理程式配對。 | Integration |
| [Case 69: Baseten 更快的一百萬情境服務](#case-69) | 當長上下文服務速度對於 Factory Droid、OpenCode 和編碼工具很重要時，可以使用此案例透過 Baseten 路由 GLM-5.2。 | Integration |
...
| [Case 83: 透過 Fireworks 設定遊標](#case-83) | 用這個案例透過 Fireworks 以最小 OpenAI 相容設定把 GLM-5.2 接入 Cursor，無須撰寫自訂客戶端程式。 | Tutorial |
| [Case 84: VulcanBench ZAI 提供者支持](#case-84) | 用這個案例在開放 benchmark harness 中透過一級支援的 ZAI provider 與專用 API key 路徑執行 GLM-5.2。 | Integration |
| [Case 85: OpenCode 高/最大推理變體](#case-85) | 用這個案例在 OpenCode 中使用 GLM-5.2 的 High / Max reasoning 變體，同時取得更可靠的 step-limit 處理。 | Integration |
| [Case 86: Z.ai編碼端點選擇](#case-86) | 用這個案例把 GLM-5.2 coding plan 流量從通用 API 路徑切到針對 coding 最佳化的 Z.ai endpoint。 | Tutorial |
| [Case 87: ZenMux 免費 GLM-5.2 API 設定](#case-87) | 用這個案例取得免費的 GLM-5.2 API key 與 base URL，再接入 Claude、Cursor、Hermes 等工具。 | Tutorial |
| [Case 93: 本體 ncode GLM 預設值](#case-93) | 如果你想把 GLM-5.2 導入 ncode 與 Noumena 風格的 agent 環境，並同時使用標準版與 1M 上下文端點以及預設模型支援，可以用這個案例。 | Integration |
| [Case 95: 克勞德代碼通過 Baseten](#case-95) | 如果你想透過 Baseten key、自訂 base URL，以及 `~/.claude/settings.json` 裡的模型映射，在 Claude Code 中執行 GLM-5.2，可以用這個案例。 | Tutorial |
| [Case 96: Deepsec Pi 代理預設值](#case-96) | 如果你想在安全 harness 中測試 GLM-5.2，可以用這個案例。`deepsec` 把它設成 Pi agent 的預設模型，並回報了具競爭力的 eval 結果。 | Integration |
| [Case 101: Baseten + Droid 安全帶快速入門](#case-101) | 如果你想透過 Baseten 與 Droid harness 快速跑通 GLM-5.2，並重用一套也適用於其他 IDE 的短設定流程，可以用這個案例。 | Tutorial |
| [Case 104: OpenAI 相容的 GLM API 工作流程](#case-104) | 如果你想在 Python 裡用一個 OpenAI 相容客戶端統一接入 GLM-5.2 的推理控制、tool calling、長上下文檢索與成本統計，可以用這個案例。 | Tutorial |
| [Case 105: Perplexity Agent API 搜尋沙箱](#case-105) | 如果你想把 GLM-5.2 接入 Perplexity 的 Agent API，並透過單一 API 呼叫取得帶搜尋能力的沙盒 agent，可以用這個案例。 | Integration |
| [Case 109: Baseten 的 280 TPS GLM API 服務](#case-109) | 如果你在意 provider 延遲表現，可以用這個案例查看 Baseten 對 GLM-5.2 高吞吐、低首 token 延遲的公開效能說法。 | Integration |
| [Case 128: Cloudflare Workers AI OpenCode 設定](#case-128) | 如果你想在不自備模型主機的情況下，透過 Cloudflare Workers AI 這條免費的 OpenAI 相容路線運行 GLM-5.2 做 coding agent，可以用這個案例。 | Tutorial |
| [Case 129: Puter.js 零設定瀏覽器客戶端](#case-129) | 如果你想在接觸 API key、帳單或後端設定之前，先用純瀏覽器原型測試 GLM-5.2，可以用這個案例。 | Tutorial |
| [Case 130: SiliconFlow 統一端點接入](#case-130) | 如果你想把 GLM-5.2 放進更大的多模型堆疊裡，可以用這個案例，因為貼文描述了一個帶 free trial credit 的單一 OpenAI 相容 SiliconFlow endpoint，同時覆蓋中文和西方模型。 | Integration |
| [Case 124: HuggingChat 建站到 HF Space 部署](#case-124) | 如果你想在幾乎免費的 personal-site flow 裡試用 GLM-5.2，從 HuggingChat 搜集資料到部署到 Hugging Face Spaces，都可以參考這個案例。 | Tutorial |
| [Case 125: DigitalOcean Inference Engine 上線](#case-125) | 如果你想透過 managed infrastructure 取得官方 provider access，而不自己托管 1M context 的模型，可以用這個案例接入 GLM-5.2。 | Integration |
| [Case 115: Command Code Fast 120-250 Tok/S 檔位](#case-115) | 如果你更在意長時程編碼任務的回應速度，而不只是最低入門價格，可以用這個案例接入更快的 GLM-5.2 檔位。 | Integration |
| [Case 116: Vercel AI 閘道快速 GLM-5.2 API](#case-116) | 如果你需要 serverless 速度和明確的 token 定價，可以用這個案例透過 Vercel AI Gateway 接入 GLM-5.2 Fast。 | Integration |
...
| [Case 234: Jatevo 折扣 GLM 接入](#case-234) | 如果你想快速掌握一條帶公開定價的 hosted GLM-5.2 接入路徑，可以看這個案例，因為 JatevoId 說其平台上 GLM 5.2 的 input 價格是 $1.40 / 1M、output 價格是 $4.40 / 1M，符合條件的 JTVO holder 還能拿到 50% 折扣。 | Integration |
| [Case 233: MI325x 上低於 0.1 美分的 GLM 服務](#case-233) | 如果你想評估 AMD 硬體上的 self-hosted GLM-5.2 inference 成本，可以看這個案例，因為 picocreator 說 4xMI325x 配置把 GLM 5.2 跑到了 1,482 tok/s，成本低於每百萬 token 0.10 美元。 | Demo |
| [Case 226: Bonsai Mac Studio 病歷分流](#case-226) | 如果你想讓一份很長的臨床病歷留在本地，同時讓 GLM-5.2 在其上做推理，可以看這個案例，因為 MaziyarPanahi 說 GLM 5.2 透過 Mac Studio 上的 Bonsai 27B 分流一份三年期病歷，並找出了埋在 17 個月前的造影風險問題。 | Demo |
| [Case 191: Hermes 建立的 LiteLLM 本地實驗室](#case-191) | 如果你想把 GLM-5.2 當作 coding agent 來搭一個本地 inference lab，可以看這個案例，因為原貼文說 Hermes Agent + GLM-5.2 把 LiteLLM、Postgres、Prometheus 和 Grafana 都接在了一套 M3 Ultra 環境上。 | Integration |
| [Case 187: 雙 M5 Max 離線無人機模擬器](#case-187) | 如果你想估算一套完全離線的 Apple Silicon GLM-5.2 agent 到底能做什麼，可以看這個案例，因為 XavierLocalAI 報告了一個 753B 配置：在兩台 128GB M5 Max 上以 26 tok/s 編寫 droneship landing simulator。 | Demo |
| [Case 186: 5x DGX Spark 生產線束](#case-186) | 如果你想判斷 5 節點 DGX Spark 配置是否已經足夠支撐 production 級 GLM-5.2 工作，可以看這個案例，因為 thatcofffeeguy 報告了在 400K context 下單流約 13.9 tok/s，以及 3 條 lane 合計 19.9 tok/s 的 live harness 結果。 | Demo |
| [Case 183: M3 Ultra ds4-eval Q4 檢查點](#case-183) | 如果你想用 ds4-eval 真正 benchmark 一台 Apple Silicon 單機上的 GLM-5.2，可以看這個案例，因為 ivanfioravanti 報告了一次 q4 運行：M3 Ultra 512GB 機器上約 16 tok/s，92 項裡過了 76 項，總時長 8 小時 53 分。 | Evaluation |
| [Case 182: 4x RTX PRO 6000 建置指南](#case-182) | 如果你想評估一套嚴肅的本地 GLM-5.2-594B 方案，可以看這個案例，因為 QingQ77 分享了一份完整的硬體與運維指南，核心是 4 張 RTX PRO 6000、透過 opencode 暴露的 API，以及給 agent 用的 sandbox VM。 | Tutorial |
| [Case 181: 4x DGX Spark QuantTrio 運行](#case-181) | 如果你想估算 4 節點 DGX Spark 叢集跑 GLM-5.2 QuantTrio 的上限，可以看這個案例，因為 Tech2Wild 給出了 200K context、單流 30 tok/s，以及 6 並發下總計 60.5 tok/s 的結果。 | Demo |
| [Case 177: 單一 M3 Ultra 4 位元影片運行](#case-177) | 如果你想估算 GLM-5.2 在 Apple Silicon 單機上的可行性，可以看這個案例，因為 ivanfioravanti 展示了在一台 M3 Ultra 512GB 機器上以約 16 tok/s 跑 4-bit 版本，並拿它和約 17 tok/s 的 q2 ds4-eval 影片做比較。 | Demo |
| [Case 173: AMD MI355X 2626 Tok/s 節點服務](#case-173) | 如果你想估算 AMD 硬體上的高吞吐 GLM-5.2 推理能力，可以看這個案例，因為 wafer_ai 表示 MI355X 達到每節點 2626 tok/s、單流 213 tok/s，而且成本比 Blackwell 低超過 2 倍。 | Demo |
| [Case 172: Vercel 閘道 287 Tok/s 無伺服器](#case-172) | 如果你想估算 GLM-5.2 經過 serverless gateway 時的真實用戶延遲，可以看這個案例，因為 wafer_ai 表示它的 GLM 5.2 Fast 路線不是只在 benchmark harness 中，而是在 Vercel AI Gateway 上跑到 287 tokens per second。 | Demo |
| [Case 171: 一鍵 RTX PRO 6000 部署](#case-171) | 如果你想估算隔離式 hosted GLM-5.2 部署的最低門檻，可以看這個案例，因為 XciD_ 表示 GLM-5.2-NVFP4 現在可在 8x RTX PRO 6000 的 Inference Endpoints 上以每小時 22 美元 one-click 部署。 | Integration |
| [Case 166: 5x ASUS GX10s 上的完整 744B](#case-166) | 如果你想估一個極端 home-lab GLM-5.2 部署的規模，可以看這個案例，因為作者說完整 744B 模型已經能在 5 台 ASUS GX10 上帶 full context 跑起來，而且已接到處理真實 workload 的 causal harness。 | Demo |
| [Case 164: 中國堆疊中的代理路由交換](#case-164) | 如果你在乎的是成本壓力而不是絕對最高品質，想把 GLM-5.2 放進 mixed-model stack 的 agent 層，可以看這個案例，因為作者說把 Sonnet 換成 GLM-5.2 之後，這個 slot 的輸入成本降到五分之一，品質大約只掉了 3%。 | Evaluation |
| [Case 156: 744B 本地硬體層](#case-156) | 如果你想更現實地評估 GLM-5.2 的本地部署門檻，可以看這個案例，因為原帖說即便量化後，2bit 也大約要 239GB、4bit 大約要 466GB，所以 256GB 以上的 RAM 或 VRAM 才算比較實際的起步線。 | Limit |
| [Case 151: 本地 NVFP4 Rust 連接埠速度為 140 Tok/s](#case-151) | 如果你想估算一套調校後的本地 GLM-5.2 部署在真實 coding 工作裡能做到什麼，可以用這個案例，因為作者回報了 140 tok/s 的 NVFP4 推理速度，以及幾分鐘內完成的 Python 到 Rust 全量移植。 | Evaluation |
| [Case 140: B300 x2 Agent 主導雙棧 Bring-Up](#case-140) | 如果你想評估一套嚴肅的自託管 GLM-5.2 部署範圍，可以用這個案例，因為整個 thread 展示了分析 agent 在不到一天內，於裸機 B300 上同時跑起 vLLM 與 SGLang 的 NVFP4 推理。 | Evaluation |
| [Case 139: oMLX M3 Ultra Prefill 加速](#case-139) | 如果你想在最近的 kernel 更新後重新檢查 Apple Silicon 本地跑 GLM-5.2 的可行性，可以用這個案例，因為回報中的 M3 Ultra 512GB prefill 速度幾乎翻倍，而且快速測試裡沒有明顯品質崩塌。 | Evaluation |
| [Case 138: 註冊即送 2000 萬 Token 額度爆發](#case-138) | 如果你想評估直接註冊拿到的免費額度，是否足夠支撐一次真正的 GLM-5.2 試用，可以用這個案例，因為貼文聲稱新帳號可拿到 2000 萬免費 tokens、免綁卡，並提供完整 OpenAI 相容接入。 | Integration |
| [Case 131: 4x DGX Spark 本地 GLM 操作手冊](#case-131) | 如果你想判斷 DGX Spark 叢集是否是現實可行的本地 GLM-5.2 路線，可以用這個案例。整理出來的指南把具體硬體成本、叢集拓撲和 vLLM 命令都對應到了 1M context 的 GLM 目標上。 | Tutorial |
| [Case 43: 輸出代幣成本比較](#case-43) | 使用此案例將 GLM-5.2 輸出代幣經濟學與 Opus、Fable 和 GPT-5.5 風格的模型進行比較。 | Evaluation |
| [Case 44: 本地近前沿硬體投資報酬率](#case-44) | 使用此案例來推理自託管類似 GLM-5.2 的模型是否可以為重度代理用戶償還硬體成本。 | Evaluation |
| [Case 45: 兩個 Mac 工作室上的 MLX](#case-45) | 使用此案例探索在 Apple 硬體和麵向 MLX 的設定上運行的本機 GLM-5.2。 | Demo |
| [Case 46: H100 每月本地部署索賠](#case-46) | 使用此案例作為成本比較提示，用於在訂閱和自託管之間進行選擇之前檢查本地部署假設。 | Evaluation |
| [Case 47: 每日積分和克勞德更換索賠](#case-47) | 使用此案例來檢查圍繞 GLM-5.2 的免費信用和替代代理敘述，同時將行銷聲明與經過驗證的工作負載適合度分開。 | Evaluation |
| [Case 48: 免費 ZCode 令牌窗口](#case-48) | 在承諾付費提供者或本地部署之前，使用此案例透過免費 ZCode 津貼測試 GLM-5.2。 | Integration |
| [Case 49: ZenMux 免費週優惠](#case-49) | 使用此案例來尋找用於 GLM-5.2 測試的時間限制的自由存取視窗。 | Integration |
| [Case 50: crofAI 每代幣定價](#case-50) | 在選擇路線之前，使用此案例比較 GLM-5.2 的第三方提供者定價。 | Integration |
| [Case 51: API價差對比](#case-51) | 將 GLM-5.2 與其他前沿實驗室和開放模型進行比較時，請使用此案例作為市場定價批評。 | Evaluation |
| [Case 64: 地下室本地推理速度](#case-64) | 在規劃離線編碼設定之前，使用此案例來估計大內存 Apple 硬體上的本地 GLM-5.2 推理吞吐量。 | Demo |
| [Case 68: Unsloth量化本地部署](#case-68) | 當完整模型權重對於普通本地硬體來說太大時，使用此案例來評估量化的 GLM-5.2 部署路徑。 | Tutorial |
| [Case 88: 兩台 M3 Ultra MLX 分散式運行](#case-88) | 用這個案例估算 GLM-5.2 8-bit 在兩台 M3 Ultra 上進行分散式 MLX 推理時的實際部署表現，再決定是否擴大 Apple Silicon 配置。 | Demo |
| [Case 89: ZCode 乘數貫穿 9 月份](#case-89) | 用這個案例透過更低的 ZCode 尖峰與離峰 multiplier，在 9 月前盡量拉長 GLM-5.2 的 plan credits。 | Integration |
| [Case 97: RTX PRO 6000 本地吞吐量](#case-97) | 如果你想估算高階本地 GLM-5.2 工作站的規模，可以用這個案例。雙 Blackwell 桌機在 4-bit 量化版本上維持了雙位數 decode 速度。 | Demo |
| [Case 98: Mac Studio API 投資報酬率現實檢查](#case-98) | 如果你想判斷為了本地 GLM-5.2 推論購買 Mac Studio 是否划算，可以用這個案例。貼文中的回本計算明顯更偏向 API 或方案存取。 | Evaluation |
| [Case 106: LiteLLM 本地中斷保存](#case-106) | 如果託管前沿 API 宕機或額度耗盡，而你又要確保交付不中斷，可以用這個案例參考本地部署的 GLM-5.2 搭配 LiteLLM 兜底。 | Demo |
| [Case 107: 個人與團隊本地投資報酬率](#case-107) | 如果你想判斷本地部署 GLM-5.2 更適合個人還是團隊，可以用這個案例做成本與組織規模判斷。 | Evaluation |
| [Case 112: 4x RTX PRO 6000 Terminal-Bench 2.0 跑分](#case-112) | 如果你在評估高階本地工作站方案，可以用這個案例把四卡 GLM-5.2 配置放到高難終端 benchmark 裡衡量。 | Evaluation |
| [Case 118: 2x RTX PRO 6000 Blackwell 本地 Crackme 解題](#case-118) | 如果你想判斷嚴肅的本地 GLM-5.2 配置能否在沒有 debugger 的情況下完成長時逆向任務，可以用這個案例。 | Demo |
...
| [Case 163: 初步網路研究平等](#case-163) | 如果你想衡量 GLM-5.2 在漏洞研究子任務上的能力，可以看這個案例，因為 Irregular 報告說，在一組範圍很窄的 cyber suite 上，它的早期內部評估可與 GPT-5.4 和 Opus 4.6 接近，同時也明確提醒 end-to-end 攻擊情境尚未測試。 | Limit |
| [Case 157: OpenRouter 削減開支技能重寫](#case-157) | 如果你想在切換 agent 模型前把遷移成本算清楚，可以看這個案例，因為某個基金團隊的 OpenRouter 試驗裡，GLM-5.2 的成本大約只有 Opus 的八分之一，但依然要重寫 skill、補 routing 邏輯，還得接受更慢、更弱一些的輸出。 | Limit |
| [Case 149: AA 冗長和推理權衡](#case-149) | 如果你想把 GLM-5.2 的 frontier 級 open-weight 智力表現和它的推理效率成本拆開看，可以用這個案例，因為 Artificial Analysis 一邊把它列為開源權重最強，一邊也指出它消耗了異常多的輸出 token。 | Evaluation |
| [Case 134: Semgrep IDOR 狹義勝出提醒](#case-134) | 如果你想把真正的安全訊號和誇大的標題敘事分開來看，可以用這個案例，因為來源指出 GLM-5.2 只是在一個 IDOR benchmark 上贏過 Claude Code，並沒有直接對上 Mythos。 | Limit |
| [Case 132: LisanBench 推理效率差距](#case-132) | 如果你想先檢查 GLM-5.2 在高推理負載任務上的表現，再決定是否把編碼優勢外推到其他場景，可以用這個案例。貼文裡的 LisanBench 結果顯示它雖然比 GLM-5 好，但相較其他開源模型仍然不夠高效。 | Limit |
| [Case 133: PrinzBench 領域錯配提醒](#case-133) | 如果你想讓 GLM-5.2 聚焦在 coding 和 agent execution，而不是法律研究，可以用這個案例，因為貼文把它較弱的 PrinzBench 分數和更強的軟體、工具使用 benchmark 做了對照。 | Limit |
| [Case 52: 無視力警告](#case-52) | 使用此案例請記住，GLM-5.2 對於需要本機視覺功能的工作流程可能不太有用。 | Limit |
| [Case 53: 現實世界的代理差距警告](#case-53) | 使用此案例可以避免過度閱讀基準測試勝利，以證明 GLM-5.2 與所有已部署的代理任務上的最佳專有模型相符。 | Limit |
| [Case 54: 安全護欄關注](#case-54) | 使用此案例提醒您在敏感網域中部署 GLM-5.2 之前執行安全性評估。 | Limit |
| [Case 55: 基準方法論批評](#case-55) | 即使整體結果有利於 GLM-5.2，也可以使用此案例來質疑基準方法。 | Limit |
| [Case 56: 尖峰時段延遲問題](#case-56) | 在切換編碼計劃或將 Claude 代碼式工作流程路由到 GLM-5.2 之前，使用此案例測試提供程式延遲。 | Limit |
| [Case 57: FutureSim 無改善結果](#case-57) | 在廣泛部署之前，使用此案例檢查編碼增益是否推廣到非編碼領域。 | Limit |
| [Case 58: 發射準備批評](#case-58) | 使用此案例將模型功能與啟動執行、文件和 API 準備分開。 | Limit |
| [Case 59: 編碼計劃價格上漲](#case-59) | 在推薦 GLM-5.2 作為低成本替代品之前，請使用此案例驗證計劃定價。 | Limit |
| [Case 67: 短時間並行工作與長代理運行](#case-67) | 使用此案例將 GLM-5.2 路由到短邊界編碼任務，同時為更深的多小時代理運行保留更強的模型。 | Limit |
...
| [Case 103: HalluHard 多圈幻覺訊號](#case-103) | 如果你想在信任其他強基準結果前，先測試 GLM-5.2 在多輪幻覺敏感任務上的表現，可以用這個案例。 | Limit |
| [Case 108: 開放式安全緊急警示](#case-108) | 如果你在做安全規劃，可以用這個案例理解開放權重 GLM-5.2 如何降低進攻性安全 agent 的實際操作門檻。 | Limit |
| [Case 126: Rust 錯誤修復通過但回合數高 7 倍](#case-126) | 如果你想把 GLM-5.2 的 code quality 優勢，和目前的 agent harness overhead 分開來看，可以用這個案例。它能修好 bug，但會比 Opus 消耗得多得多。 | Evaluation |
| [Case 114: Braintrust 模型替換成本警示](#case-114) | 用這個案例避免想當然地認為，在真實 agentic coding workflow 裡把模型換成更便宜的選項後，品質還能維持不變。 | Evaluation |
...
**如果你想評估 GLM-5.2 在 public-equity research agent 上的表現，可以看這個案例，因為 karinanguyen 說 DiligenceBench 把 GLM 5.2 排到接近最前面，並證明 finance harness 能讓強模型同時變得更強也更便宜。**

karinanguyen 把 DiligenceBench 介紹成一套以 rubric 為核心的 public-equity research 評估，並表示在這條 finance harness 上，Meta Muse Spark 1.1 以 57.4% 領先，而 GLM 5.2 緊跟在 Sonnet 4.6 與 GPT-5.6 Sol 之後。貼文同時主張，generic tools 對強模型的增益最大，較弱的模型反而更需要 domain-specific scaffolding；也正因如此，這條 finance harness 把 price-performance frontier 推到新的位置，讓 GLM 5.2 在絕對表現上特別突出，而 MiniMax M3 則在效率面看起來最強。

Type: Evaluation | Date: 2026-07-17

---
<a id="case-227"></a>
### Case 227: [Gargantua WebGL Raytracer 勝出](https://x.com/AlicanKiraz0/status/2077885419744612597) (by [@AlicanKiraz0](https://x.com/AlicanKiraz0))

**如果你想在偏物理的單檔 browser build 上 benchmark GLM-5.2，可以看這個案例，因為 AlicanKiraz0 說 GLM 5.2 Max 在 Gargantua geodesic raytracer 任務裡更好地兼顧了數值正確性與 real-time rendering discipline。**

AlicanKiraz0 描述的是一個 one-file raw WebGL2 任務，要實作帶有 RK4 null-geodesic integration、accretion disk、gravitational lensing、redshift、Doppler effects、camera controls 與完整 control panel 的 Schwarzschild black-hole renderer。貼文強調，決定勝負的不只是 visuals，而是 numerical correctness、boundary handling 與 solver discipline；最終 GLM 5.2 Max 拿到 92/100，高於 GPT5.6 Sol Ultra 的 90、Kimi K3 Thinking Max 的 85，以及 Fable 5 Max 的 80。

Type: Evaluation | Date: 2026-07-16

<a id="case-223"></a>
### Case 223: [Intelligence Index Token 效率落差](https://x.com/ArtificialAnlys/status/2077466596528832678) (by [@ArtificialAnlys](https://x.com/ArtificialAnlys))

**如果你想為長時程 benchmark 工作負載規劃 GLM-5.2 預算，可以看這個案例，因為 Artificial Analysis 說，GLM-5.2 Max 在 Intelligence Index 每個任務上的平均輸出 token 約為 43K，而 Inkling 是 25K，Kimi K2.6 和 DeepSeek v4 Pro Max 也都更低。**

Artificial Analysis 這次拆開來看的不是 leaderboard 分數，而是輸出 token 消耗，因此它直接把 GLM-5.2 放到了同類 open-weight 模型裡偏貴的一側。貼文對比了 Inkling 的 25K 平均輸出 token、GLM-5.2 Max 的 43K、Kimi K2.6 的 38K，以及 DeepSeek v4 Pro Max 的 37K；如果團隊已經接受 GLM 的品質，但還要預估 agent loop 的 token 消耗，這就是一條很實用的效率提醒。
...
<a id="case-199"></a>
### Case 199: [Epoch 開放權重指數領先](https://x.com/EpochAIResearch/status/2074894535558300103) (by [@EpochAIResearch](https://x.com/EpochAIResearch))
...
---
<a id="case-196"></a>
### Case 196: [Databricks 內部線束評估](https://x.com/alighodsi/status/2074996561306955958) (by [@alighodsi](https://x.com/alighodsi))
...
---
<a id="case-190"></a>
### Case 190: [NatureBench 公開重量級亞軍](https://x.com/OkhayIea/status/2074498200262889837) (by [@OkhayIea](https://x.com/OkhayIea))
...
---
<a id="case-189"></a>
### Case 189: [終端-工作台 45-任務成本權衡](https://x.com/Aiswarya_Sankar/status/2074554064856314219) (by [@Aiswarya_Sankar](https://x.com/Aiswarya_Sankar))
...
---
<a id="case-188"></a>
### Case 188: [Harvey LAB-AA 法律代理領帶](https://x.com/ArtificialAnlys/status/2074541975186165887) (by [@ArtificialAnlys](https://x.com/ArtificialAnlys))
...
---
<a id="case-184"></a>
### Case 184: [AutomationBench-AA 開放式重量領先](https://x.com/ArtificialAnlys/status/2074194764510208230) (by [@ArtificialAnlys](https://x.com/ArtificialAnlys))
...
---
<a id="case-178"></a>
### Case 178: [三體模擬器基準獲勝](https://x.com/AlicanKiraz0/status/2073823792543998170) (by [@AlicanKiraz0](https://x.com/AlicanKiraz0))
...
---
<a id="case-167"></a>
### Case 167: [GameDevBench 333 任務開源負責人](https://x.com/iamwaynechi/status/2073081777091182633) (by [@iamwaynechi](https://x.com/iamwaynechi))
...
<a id="case-175"></a>
### Case 175: [遊標雙擺記分卡](https://x.com/AlicanKiraz0/status/2073386918813778143) (by [@AlicanKiraz0](https://x.com/AlicanKiraz0))
...
<a id="case-162"></a>
### Case 162: [VulcanBench 10 項任務 80% 平局](https://x.com/morganlinton/status/2072689409011679642) (by [@morganlinton](https://x.com/morganlinton))
...
<a id="case-159"></a>
### Case 159: [SWE-重新基準 51.1% 檢查點](https://x.com/ibragim_bad/status/2072318238407483593) (by [@ibragim_bad](https://x.com/ibragim_bad))
...
<a id="case-154"></a>
### Case 154: [推出 Darkly Edge-Case 以 40/41 獲勝](https://x.com/composio/status/2072355937415827950) (by [@composio](https://x.com/composio))
...
<a id="case-146"></a>
### Case 146: [CyberBench 開放重量補丁亞軍](https://x.com/ValsAI/status/2072099011398627639) (by [@ValsAI](https://x.com/ValsAI))
...
<a id="case-1"></a>
### Case 1: [人工智慧分析智能指數](https://x.com/ArtificialAnlys/status/2067135640249209175) (by [@ArtificialAnlys](https://x.com/ArtificialAnlys))
...
<a id="case-3"></a>
### Case 3: [設計競技場第一名](https://x.com/Designarena/status/2066940737011560652) (by [@Designarena](https://x.com/Designarena))
...
<a id="case-4"></a>
### Case 4: [FrontierSWE 結果](https://x.com/ProximalHQ/status/2066939701026787583) (by [@ProximalHQ](https://x.com/ProximalHQ))
...
<a id="case-5"></a>
### Case 5: [DeepSWE 開源負責人](https://x.com/AiBattle_/status/2066938378512126024) (by [@AiBattle_](https://x.com/AiBattle_))
...
<a id="case-6"></a>
### Case 6: [終端工作台超過 80%](https://x.com/cline/status/2066951439793242193) (by [@cline](https://x.com/cline))
...
<a id="case-7"></a>
### Case 7: [SWELancer 與 GPT-5.5 的比較](https://x.com/gosrum/status/2067153091842203676) (by [@gosrum](https://x.com/gosrum))
...
<a id="case-8"></a>
### Case 8: [BridgeBench 滿分訊號](https://x.com/bridgemindai/status/2065874542321426819) (by [@bridgemindai](https://x.com/bridgemindai))
...
<a id="case-10"></a>
### Case 10: [KernelBench-Hard 沒有捷徑](https://x.com/elliotarledge/status/2065735912370417760) (by [@elliotarledge](https://x.com/elliotarledge))
...
<a id="case-11"></a>
### Case 11: [Runescape 長椅追趕](https://x.com/maxbittker/status/2066985743197577433) (by [@maxbittker](https://x.com/maxbittker))
...
<a id="case-60"></a>
### Case 60: [KernelBench 硬核與巨型 GPU 編碼](https://x.com/elliotarledge/status/2068177175640240323) (by [@elliotarledge](https://x.com/elliotarledge))
...
<a id="case-70"></a>
### Case 70: [DeepSWE 高強度模式開源領先](https://x.com/datacurve/status/2068473057107476680) (by [@datacurve](https://x.com/datacurve))
...
<a id="case-72"></a>
### Case 72: [LLM 辯論基準第二名](https://x.com/LechMazur/status/2068428300460974279) (by [@LechMazur](https://x.com/LechMazur))
...
<a id="case-76"></a>
### Case 76: [AA-Omniscience 幻覺率](https://x.com/yuhasbeentaken/status/2068259921519423855) (by [@yuhasbeentaken](https://x.com/yuhasbeentaken))
...
<a id="case-90"></a>
### Case 90: [GDPval-AA 代理工作指數](https://x.com/ArtificialAnlys/status/2069121548670406947) (by [@ArtificialAnlys](https://x.com/ArtificialAnlys))
...
<a id="case-94"></a>
### Case 94: [遊戲開發競技場亞軍](https://x.com/Designarena/status/2069166634976371084) (by [@Designarena](https://x.com/Designarena))
...
<a id="case-120"></a>
### Case 120: [PostTrainBench 可靠性領先](https://x.com/hrdkbhatnagar/status/2070244540108423427) (by [@hrdkbhatnagar](https://x.com/hrdkbhatnagar))
...
<a id="case-121"></a>
### Case 121: [Fireworks + Faros 211 項儲存庫任務評測](https://x.com/FireworksAI_HQ/status/2070181898962534570) (by [@FireworksAI_HQ](https://x.com/FireworksAI_HQ))
...
<a id="case-110"></a>
### Case 110: [AA-Briefcase 每任務耗時前沿](https://x.com/ArtificialAnlys/status/2069914443639635978) (by [@ArtificialAnlys](https://x.com/ArtificialAnlys))
...
<a id="case-111"></a>
### Case 111: [Code Arena 前端對戰優勢](https://x.com/arena/status/2069885722333769963) (by [@arena](https://x.com/arena))
...
<a id="case-113"></a>
### Case 113: [SWE Atlas 程式庫問答亞軍](https://x.com/ScaleAILabs/status/2069864932913631617) (by [@ScaleAILabs](https://x.com/ScaleAILabs))
...
---
<a id="case-257"></a>
### Case 257: [OpenCodex 模型切換工作流](https://x.com/vista8/status/2079239701391675404) (by [@vista8](https://x.com/vista8))

**如果你想在以 Codex 為中心的 coding loop 裡替 GLM-5.2 做路由，而不是被鎖定在單一模型上，可以看這個案例，因為 vista8 說 OpenCodex 讓同一套環境可以在 GLM 5.2、Kimi K3、GPT-5.6 Sol 和 Grok 4.5 之間切換，用於前端設計、後端工作與即時 X 搜尋。**

vista8 說，Codex 依然是自己日常最常用的 coding tool，但前端審美是它的弱點，所以工作流轉到了能按需切換模型的 OpenCodex 專案裡。貼文給出的路由說明很實用：前端設計走透過 OAuth 接入的 Kimi K3，後端工作切到 GPT-5.6 Sol，即時 X 搜尋交給 Grok 4.5，同時把 GLM 5.2 當成同一 coding surface 裡的另一個可替換選項。因此，這更像是一條面向 coding agents 的具體模型路由工作流，而不是單純的模型排名觀點。

Type: Integration | Date: 2026-07-20

---
<a id="case-255"></a>
### Case 255: [Hermes 11 代理混合實驗室](https://x.com/MichaelGannotti/status/2079168568478912834) (by [@MichaelGannotti](https://x.com/MichaelGannotti))

**如果你想圍繞 GLM-5.2 搭一套按角色分工的多代理實驗室，而不是只用一個單體助手，可以看這個案例，因為 MichaelGannotti 說一套 11 代理的 Hermes 配置，會在 DGX Spark、Ryzen 工作站和雲端模型之間動態路由任務，其中就包括用於軟體、研究、行銷與協調工作的 GLM 5.2。**

MichaelGannotti 給出了一套很具體的實驗室架構：一台帶 128GB unified memory 的 DGX Spark 作為主要本地推理伺服器，一台 Ryzen 9 Halo Strix 機器運行 8 個負責工程與組織角色的 Hermes agents，另一台 Ryzen 7 Windows 機器再運行 3 個面向內容與微軟生態工作的 agents。在這套配置裡，每個 agent 會依任務在雲端的 GPT-5.6、GLM 5.2、Grok 4.5，以及本地的 Qwen、Nemotron、Gemma 模型之間做選擇，因此這則貼文更像是一份混合本地加雲端 agent 團隊的操作模型參考，而不是含糊的效率宣傳。
...
**如果你想評估一個 local-first 的 GLM-5.2 coding agent，可以看這個案例，因為 chenzeling4 說 LM Studio Bionic 把 GLM 5.2 跟本地文件 sandbox、inline code diff、rollback checkpoint，以及裝置端語音轉錄整合在一起。**

chenzeling4 把 LM Studio Bionic 描述成一個圍繞 open model 打造的獨立 agent，可依任務選擇本地或雲端算力，但兩種路徑都維持 zero-retention 定位。貼文指出，coding 由 GLM 5.2 和 Kimi K2.7 Code 負責，文件會在 sandbox 中處理並自動留下 rollback checkpoint，程式修改會直接以 inline diff 顯示，而語音轉錄則透過 Voxtral 保持在裝置端完成。這讓它成為一條具體的本地 agent 工作流更新，而不只是泛泛的模型可用性訊號。

Type: Integration | Date: 2026-07-17

---
<a id="case-236"></a>
### Case 236: [Claude Code 網頁開發品質優勢](https://x.com/Lumenix0/status/2078241726897230164) (by [@Lumenix0](https://x.com/Lumenix0))

**如果你想比較 first-pass 網頁開發品質，而不是只看完成速度，可以看這個案例，因為 Lumenix0 說 GLM 5.2 在 Claude Code 裡，於三個真實任務上都在設計品質與功能完整度方面勝過了 Codex 裡的 GPT 5.5。**

Lumenix0 總結了一組 head-to-head 測試：用 Claude Code 裡的 GLM 5.2 對上 Codex 裡的 GPT 5.5，任務包含網站改版、React bug 修復，以及 Kanban board 建置。貼文表示，GPT 在每個任務上都更快完成，但 GLM 產出了更好的 redesign，在 bug 修復上給出同樣正確但更清楚的解釋，還在一次生成中交付了唯一完整可用的 Kanban board，包含任務命名、優先級、狀態切換與 reset button。相同來源也補充，這三組測試合計只用掉 16 美元方案每週額度的 7%。

Type: Evaluation | Date: 2026-07-17

<a id="case-168"></a>
### Case 168: [Synthwave Hard-Slice Ensemble 售價 $2.66 美元](https://x.com/TracNetwork/status/2073038214592360522) (by [@TracNetwork](https://x.com/TracNetwork))
...
**如果你想在付費訂閱 frontier 模型前先驗證本地 coding-agent stack，可以看這個案例，因為 comma_ai 說他們已經移除 Anthropic，並發現 GLM 5.2 加 OpenCode 的 agentic coding 流程更好用。**

comma_ai 說，自家的 GLM deployment 就跑在訓練 open-source driving agent 的機器旁邊，因此這更像是本地所有權訊號，而不是單純偏愛 cloud。thread 明確把 GLM 5.2 和 OpenCode 綁在一起，並說團隊在把 Anthropic 移出 stack 後，日常 agentic coding 體驗反而更好，所以它更適合被視為一條實作型的 local-first workflow 參考，而不是泛泛的開源口號。

Type: Demo | Date: 2026-07-16

<a id="case-212"></a>
### Case 212: [Dell Hub GLM Agent 教學](https://x.com/juanjucm/status/2076714987569963508) (by [@juanjucm](https://x.com/juanjucm))

**如果你想為開放權重訓練工作流搭一套 GLM-5.2 coding agent，可以看這個案例，因為 juanjucm 表示，一篇新指南把 Dell Enterprise Hub 新增 GLM-5.2-FP8 目錄更新，與一套圍繞該模型構建 agent 的逐步搭建流程放在了一起。**

juanjucm 表示，他寫了一篇公開指南，介紹如何搭建一個基於 GLM-5.2 的 coding agent，用來訓練兩個小型語言模型，並把這份教學與 Dell Enterprise Hub 將 GLM-5.2-FP8 加入目錄這件事連在一起。連結到的 Hugging Face 文章給出了公開教學入口，而貼文本身則把這套 stack 定位為一條面向動手訓練和 agent 實驗的 open-weight 路線，而不是一條泛泛的可用性通知。
...
<a id="case-211"></a>
### Case 211: [8xB200 開放權重報告流水線](https://x.com/TheZachMueller/status/2076746035758502275) (by [@TheZachMueller](https://x.com/TheZachMueller))

**如果你想把 GLM-5.2 作為主筆，接進一條貼近本地部署的報告流水線，可以看這個案例，因為 TheZachMueller 表示，一台 8xB200 節點按 4/4 切分後，可以讓 GLM 5.2 NVFP4 負責報告生成、Kimi K2.7 Code 負責檢索，以相對 Claude API 幾乎只是零頭的成本產出一份更紮實的 36 頁報告。**

TheZachMueller 表示，他花了一個週末調參後，把一條真實工作流水線從 Claude Code 遷到了 Pi dot dev 加 open-weight stack 上。發布的配置把一台 8xB200 節點拆成兩半：GLM 5.2 NVFP4 作為主 agent 和 driver，Kimi K2.7 Code 作為 retriever，最終產出一份 36 頁報告，而 Claude 的版本只有 21 頁。貼文也把 tradeoff 說得很清楚：整體週轉時間從大約 20 分鐘變成約 30 到 40 分鐘，而品質上最大的修正，是不要反覆總結來源文章，而是把完整文章保存在磁碟上以便做更深的檢索。
...
**如果你想把 GLM-5.2 當成一個研究密集型 frontend fixer，放進多代理網站改版流程裡測試，可以看這個案例，因為 spettrotoken 表示在 Fable 5 和 GPT-5.5 都失敗之後，GLM 5.2 透過整合好的 web scraping 與 data fetching 工具，做出了可在 Firefox 運作的跨瀏覽器 Liquid Glass 實作。**

spettrotoken 表示，一次真實上線中的 Spettro 網站改版被拆成四個 Spettro instance，各自負責不同的 frontend 區塊，而 GLM-5.2 則負責最難的視覺元件：一個通常會在 Firefox 壞掉的折射式 Liquid Glass 效果。貼文指出，GLM 5.2 會自行搜尋 web、閱讀 CSS 與 SVG filter workaround、逆向拆解這個效果，最後交付一個已部署到 live site 的可用跨瀏覽器實作；更大範圍的改版則還有 Kimi K2.7 與平行 sub-agent 一起支援。
...
<a id="case-194"></a>
### Case 194: [CuTeDSL 內核技能開源](https://x.com/SubhoGhosh02/status/2074466050557739469) (by [@SubhoGhosh02](https://x.com/SubhoGhosh02))
...
---
<a id="case-180"></a>
### Case 180: [Hermes SSD 恢復技能循環](https://x.com/ShankhadeepSho1/status/2073658918937473444) (by [@ShankhadeepSho1](https://x.com/ShankhadeepSho1))
...
---
<a id="case-174"></a>
### Case 174: [角色路由的重型編碼器堆疊](https://x.com/denizirgin/status/2073462071639876004) (by [@denizirgin](https://x.com/denizirgin))
...
<a id="case-155"></a>
### Case 155: [Cotal 四代理 TUI 循環](https://x.com/silvanrec/status/2072335315822403656) (by [@silvanrec](https://x.com/silvanrec))
...
<a id="case-153"></a>
### Case 153: [舊版遷移成本削減試點](https://x.com/chamath/status/2072390507628540213) (by [@chamath](https://x.com/chamath))
...
<a id="case-150"></a>
### Case 150: [Mac Studio瀏覽器-使用本機循環](https://x.com/MaziyarPanahi/status/2071955191260151862) (by [@MaziyarPanahi](https://x.com/MaziyarPanahi))
...
<a id="case-148"></a>
### Case 148: [Gumloop 代理交換成本削減](https://x.com/aronkor/status/2072032854675218538) (by [@aronkor](https://x.com/aronkor))
...
<a id="case-13"></a>
### Case 13: [一小時四十二分鐘重構循環](https://x.com/KELMAND1/status/2066012493315723610) (by [@KELMAND1](https://x.com/KELMAND1))
...
<a id="case-14"></a>
### Case 14: [OpenCode Bug 修復與實作測試](https://x.com/altudev/status/2065868921341632881) (by [@altudev](https://x.com/altudev))
...
<a id="case-15"></a>
### Case 15: [OpenCode 復古電玩演練](https://x.com/AskVenice/status/2067047775783534789) (by [@AskVenice](https://x.com/AskVenice))
...
<a id="case-16"></a>
### Case 16: [HTML5 實體模擬競賽](https://x.com/atomic_chat_hq/status/2067038851139334588) (by [@atomic_chat_hq](https://x.com/atomic_chat_hq))
...
<a id="case-17"></a>
### Case 17: [個人網站 UI UX 構建](https://x.com/anshuc/status/2067034697704857615) (by [@anshuc](https://x.com/anshuc))
...
<a id="case-18"></a>
### Case 18: [AI合約審查產品構建](https://x.com/laozhang2579/status/2066339734956499301) (by [@laozhang2579](https://x.com/laozhang2579))
...
<a id="case-19"></a>
### Case 19: [ZCode 目標功能可實現更大的開發目標](https://x.com/zcode_ai/status/2066236605917188228) (by [@zcode_ai](https://x.com/zcode_ai))
...
<a id="case-20"></a>
### Case 20: [使用 GLM-5.2 建構的 ZCode 的 Linux 包裝器](https://x.com/gosrum/status/2066484949755269510) (by [@gosrum](https://x.com/gosrum))
...
<a id="case-21"></a>
### Case 21: [計算機使用技能包裝](https://x.com/0xSero/status/2065952130054382079) (by [@0xSero](https://x.com/0xSero))
...
<a id="case-22"></a>
### Case 22: [ZCode 3.0代理開發環境審查](https://x.com/laogui/status/2066190262993559963) (by [@laogui](https://x.com/laogui))
...
<a id="case-62"></a>
### Case 62: [具有本地服務的 OpenCode Harness](https://x.com/PatrickToulme/status/2068134212587184442) (by [@PatrickToulme](https://x.com/PatrickToulme))
...
<a id="case-65"></a>
### Case 65: [Fast-RLM 長上下文指令注入](https://x.com/neural_avb/status/2067992817625088439) (by [@neural_avb](https://x.com/neural_avb))
...
<a id="case-66"></a>
### Case 66: [DeepAgents 程式碼開放式線束試用](https://x.com/sydneyrunkle/status/2067947260369854830) (by [@sydneyrunkle](https://x.com/sydneyrunkle))
...
<a id="case-77"></a>
### Case 77: [生產級行銷 Agent 棧路由策略](https://x.com/DeRonin_/status/2068303752671477820) (by [@DeRonin_](https://x.com/DeRonin_))
...
<a id="case-80"></a>
### Case 80: [M3 究極精靈寶可夢 紅色目標跑](https://x.com/hxiao/status/2068800750554378434) (by [@hxiao](https://x.com/hxiao))
...
<a id="case-91"></a>
### Case 91: [Cline Repo 錯誤修復攤牌](https://x.com/cline/status/2069171146994729078) (by [@cline](https://x.com/cline))
...
<a id="case-102"></a>
### Case 102: [OpenInspect FP8 後台代理](https://x.com/colemurray/status/2069485572339707938) (by [@colemurray](https://x.com/colemurray))
...
<a id="case-145"></a>
### Case 145: [OpenCode + Fireworks 降本遷移](https://x.com/SeekingN0rth/status/2071484711327985696) (by [@SeekingN0rth](https://x.com/SeekingN0rth))
...
<a id="case-142"></a>
### Case 142: [Cline 推理開關帶來的 Harness 差值](https://x.com/akshay_pachaar/status/2071638409022763292) (by [@akshay_pachaar](https://x.com/akshay_pachaar))
...
<a id="case-136"></a>
### Case 136: [Cursor + Fireworks 4.55 億 Token 實戰測試](https://x.com/robinebers/status/2071246749210190132) (by [@robinebers](https://x.com/robinebers))
...
<a id="case-135"></a>
### Case 135: [Devin Desktop Harness 與 Skill 可攜性](https://x.com/lily_gpupoor/status/2071297351801794850) (by [@lily_gpupoor](https://x.com/lily_gpupoor))
...
<a id="case-127"></a>
### Case 127: [Pi 內聯 GLM 審閱者](https://x.com/xpasky/status/2070831715518460177) (by [@xpasky](https://x.com/xpasky))
...
<a id="case-117"></a>
### Case 117: [OpenCode Go 重構首輪取勝](https://x.com/vedovelli74/status/2069889605969592500) (by [@vedovelli74](https://x.com/vedovelli74))
...
<a id="case-119"></a>
### Case 119: [Claude Code + Cursor 預設執行成本 3.36 美元](https://x.com/clairevo/status/2069828122640548204) (by [@clairevo](https://x.com/clairevo))
...
## 🎮 上手演示與作品展示
<a id="case-229"></a>
### Case 229: [Hyperagent 個人首頁作品集對決](https://x.com/arsh_goyal/status/2077764207945416949) (by [@arsh_goyal](https://x.com/arsh_goyal))

**如果你想在真實 browser-based agent 任務裡比較 GLM-5.2 和其他 open model，可以看這個案例，因為 arsh_goyal 把 GLM 5.2、DeepSeek V4、Kimi K2.6 和 Qwen 3.7 並排放到 Hyperagent 上，用公開資料自動生成個人作品集。**

arsh_goyal 說，每個模型都在一台帶真實 browser 的獨立 cloud machine 上執行，先讀取作者的 YouTube、LinkedIn 和 X 資料，再從同一條 one-line prompt 生成網站。貼文還公開了每次 run 的 cost 和 duration，並在回覆裡附上了 video 和 prompt，因此這比普通 screenshot 或 leaderboard repost 更接近一條真正可複現的 hands-on 對比。
...
**如果你想圍繞 GLM-5.2 的規劃與設計優勢，原型化一條 one-prompt app generation 工作流，可以看這個案例，因為 nutlope 表示，LlamaCoder v4 已圍繞 GLM 5.2 重構，改進了解析與規劃，並且現在在一套免費開源 stack 上直接交付 WebAssembly renderer。**

nutlope 表示，LlamaCoder v4 現在以 GLM 5.2 為中心，把 UI 層切到 Base UI with shadcn，改進了解析、規劃和 app 設計，並新增了一個基於 WebAssembly 的 renderer。這個專案被描述為免費、開源、由 Together 驅動，所以它是一條已經交付的具體 demo，而不只是對模型品質的主觀看法。
...
<a id="case-202"></a>
### Case 202: [命令代碼太空射擊遊戲功能獲勝](https://x.com/CommandCodeAI/status/2075264795817972107) (by [@CommandCodeAI](https://x.com/CommandCodeAI))
...
<a id="case-200"></a>
### Case 200: [ZCode 任天堂 DS 模擬器](https://x.com/0xSero/status/2074870153267818638) (by [@0xSero](https://x.com/0xSero))
...
---
<a id="case-192"></a>
### Case 192: [命令代碼 Flappy Bird UX Split](https://x.com/MrAhmadAwais/status/2074536879308026031) (by [@MrAhmadAwais](https://x.com/MrAhmadAwais))
...
---
<a id="case-161"></a>
### Case 161: [REAP NVFP4 魔術方塊一擊](https://x.com/RoundtableSpace/status/2072700573145788914) (by [@RoundtableSpace](https://x.com/RoundtableSpace))
...
<a id="case-158"></a>
### Case 158: [OMP 中繼 iPhone 用戶端](https://x.com/mov_axbx/status/2072192903762288721) (by [@mov_axbx](https://x.com/mov_axbx))
...
<a id="case-144"></a>
### Case 144: [開源 DevRel 研究 Agent](https://x.com/Astrodevil_/status/2071572680470655253) (by [@Astrodevil_](https://x.com/Astrodevil_))
...
<a id="case-123"></a>
### Case 123: [Recast 六版本落地頁迭代流程](https://x.com/nutlope/status/2070199649818779914) (by [@nutlope](https://x.com/nutlope))
...
<a id="case-23"></a>
### Case 23: [可玩的密室一擊](https://x.com/aimlapi/status/2066996493257408639) (by [@aimlapi](https://x.com/aimlapi))
...
<a id="case-24"></a>
### Case 24: [三個真實的構建結果參差不齊](https://x.com/bridgemindai/status/2065840871929442319) (by [@bridgemindai](https://x.com/bridgemindai))
...
<a id="case-25"></a>
### Case 25: [ZCode 中的超級瑪利歐克隆](https://x.com/ivanfioravanti/status/2066272681406980208) (by [@ivanfioravanti](https://x.com/ivanfioravanti))
...
<a id="case-26"></a>
### Case 26: [月球登陸器競賽](https://x.com/ivanfioravanti/status/2066193134984319173) (by [@ivanfioravanti](https://x.com/ivanfioravanti))
...
<a id="case-27"></a>
### Case 27: [一鍵設計競技場創建](https://x.com/grx_xce/status/2066951779154374907) (by [@grx_xce](https://x.com/grx_xce))
...
<a id="case-28"></a>
### Case 28: [研究論文理解工作流程](https://x.com/askalphaxiv/status/2066996976445706745) (by [@askalphaxiv](https://x.com/askalphaxiv))
...
<a id="case-29"></a>
### Case 29: [受限制的詩歌比較](https://x.com/emollick/status/2067056226337186146) (by [@emollick](https://x.com/emollick))
...
<a id="case-30"></a>
### Case 30: [設計感範例](https://x.com/0xSero/status/2067074877941796994) (by [@0xSero](https://x.com/0xSero))
...
<a id="case-71"></a>
### Case 71: [Temple Run 體素遊戲單次生成](https://x.com/pseudokid/status/2068431546143649829) (by [@pseudokid](https://x.com/pseudokid))
...
<a id="case-78"></a>
### Case 78: [OpenCode Go 單次生成案例集](https://x.com/LyalinDotCom/status/2068378281636982990) (by [@LyalinDotCom](https://x.com/LyalinDotCom))
...
<a id="case-81"></a>
### Case 81: [太空侵略者一鍵構建](https://x.com/DeryaTR_/status/2068803754611069128) (by [@DeryaTR_](https://x.com/DeryaTR_))
...
<a id="case-82"></a>
### Case 82: [OpenCode 復原實驗室一次性](https://x.com/threepointone/status/2068718370581536816) (by [@threepointone](https://x.com/threepointone))
...
<a id="case-92"></a>
### Case 92: [開啟設計參考 URL 重建](https://x.com/OpenDesignHQ/status/2069046584134778995) (by [@OpenDesignHQ](https://x.com/OpenDesignHQ))
...
<a id="case-99"></a>
### Case 99: [交易台成本品質測試](https://x.com/atomic_chat_hq/status/2069171121044513273) (by [@atomic_chat_hq](https://x.com/atomic_chat_hq))
...
<a id="case-100"></a>
### Case 100: [克勞德拒絕後的勒德分子遊戲](https://x.com/atmoio/status/2069559053114577088) (by [@atmoio](https://x.com/atmoio))
...
**如果你想在 Orbit 裡試用 GLM-5.2，同時不自己管理單獨的 provider 憑證，可以看這個案例，因為 Orbit Editor v0.4.0 說其新的 Orbit Provider 會把 GLM-5.2 直接暴露在編輯器裡，並以 50% 折扣上線。**

Orbit Editor 的 v0.4.0 發布引入了一層面向開源模型的內建 provider，目標是把單獨設定 API key 和 provider 的工作，收斂到一個編輯器工作流裡。貼文表示，使用者可以在同一個介面裡選模型、開始編碼、追蹤用量並管理成本，而 GLM-5.2 則以 5 折價格上線。因此，這是一條很具體的編輯器整合與存取路徑，適合那些想低摩擦試用 GLM 的團隊。

Type: Integration | Date: 2026-07-20

---
<a id="case-256"></a>
### Case 256: [pen.dev 並行設計評測](https://x.com/tomkrcha/status/2079253857834828230) (by [@tomkrcha](https://x.com/tomkrcha))

**如果你想在同一套本地工作流裡，把 GLM-5.2 和偏設計導向的同類模型並排比較，可以看這個案例，因為 tomkrcha 說 pen.dev 能讓 GLM 5.2 和 ChatGPT、Claude、Cursor Composer、OpenCode Go、Kimi、Grok、Gemini、Qwen 等模型一起並行運行，直接在桌面 codebase 上做設計任務。**

tomkrcha 把 pen.dev 描述為一層即時設計 agent surface，而不是單模型 demo。貼文表示，開發者可以直接登入自己已有的 ChatGPT 或 Claude 訂閱，接入 Cursor Composer、GitHub Copilot、OpenCode Go、Grok 4.5、Kimi、MiniMax、GLM 5.2、Gemini、Qwen，再把它們混搭到同一個本地專案裡的前端設計任務上。因此，這更像是一個面向 UI-heavy 工作的 GLM-5.2 橫向評測介面，而不是另一張孤立的 benchmark 截圖。

Type: Integration | Date: 2026-07-20

<a id="case-239"></a>
### Case 239: [TokenRouter 免費 GLM API 視窗](https://x.com/meliasiih/status/2078180641468985564) (by [@meliasiih](https://x.com/meliasiih))

**如果你想快速拿到一條短期免費的 GLM-5.2 API 路徑，可以看這個案例，因為 meliasiih 說 TokenRouter 提供到 2026-07-25 為止的免費存取，並且流程包含簡單註冊、API key 與公開 base URL。**

meliasiih 寫得很具體：先用 email 或 X 註冊、完成帳號驗證、生成 API key、選擇 GLM-5.2，然後把客戶端指向 `https://api.tokenrouter.com/v1`。來源也直接給了操作期限，說這個免費存取計畫會持續到 2026-07-25，因此它不是泛泛的可用性消息，而是一條團隊可以立即採用的限時 hosted access 路徑。

Type: Tutorial | Date: 2026-07-17

---
<a id="case-238"></a>
### Case 238: [Agentuity Wafer GLM 閘道](https://x.com/wafer_ai/status/2078186124258984374) (by [@wafer_ai](https://x.com/wafer_ai))

**如果你想把 GLM-5.2 接進 Agentuity gateway stack，可以看這個案例，因為 wafer_ai 說它的 serverless 路線現在已在 Agentuity 上提供 GLM 5.2，regular 和 Fast tier 都支援約 100 到 250 tok/s 與 1M context。**

wafer_ai 表示，市場上速度最快的 serverless GLM 5.2 路線，現在已經能透過 Agentuity AI Gateway 取得，而 Wafer 會在那裡同時提供 regular 與 Fast 兩個變體。這則貼文給出的不是模糊的 availability note，而是可直接拿來規劃部署的設定輪廓：兩個 tier 都有約 100 到 250 tokens per second，以及 1M-token context window。
...
**如果你想在保留 check-run workflow 的同時壓低 PR review agent 成本，可以看這個案例，因為 kayvz 說 Macroscope 的 Check Run Agents 現在可以直接從一般 `.md` repo 設定裡選用 GLM 5.2。**

kayvz 說，新的 model options 會直接出現在配置 check-run `.md` 檔案的地方，因此這比單純 availability 貼文更接近真實整合面。thread 也明確把 pull request 裡的 custom code-review agents 當作使用情境，所以對於已經透過 Macroscope 跑 review automation、但想加入 open-weight 選項的團隊來說，它是一條很具體的 integration surface。
...
**如果你想 benchmark 一個高速 hosted GLM-5.2 endpoint，可以看這個案例，因為 asterailabs 說 Aster Inference 把來自 research-agent 優化的 API 路由以 281 tokens per second 提供給 GLM 5.2。**

Aster 把自己的產品定位為一條由 AI research agents 打磨出來的 inference API，並給出了具體 throughput 數字，而不是只說 launch 口號。貼文寫到 GPU 上 gpt-oss-120b 可達 644 tps、GLM 5.2 可達 281 tps，同時還說公司會把研究系統裡的 inference 經驗直接回灌到產品改進裡，因此這條路徑很適合那些想先比 hosted provider，而不是直接從零開始 self-hosting 的團隊。
...
**如果你想把 GLM-5.2 接進現有 TrueFoundry AI Gateway stack，可以看這個案例，因為 wafer_ai 說這條 native provider integration 從 GLM 5.2 和 GLM 5.2 Fast 開始，而且不需要改動閘道其餘部分。**

wafer_ai 說，已經在用 TrueFoundry AI Gateway 的團隊，可以在不調整 stack 其他部分的前提下直接接入 Wafer models。rollout 從 GLM 5.2 和 GLM 5.2 Fast 起步，貼文還把 Wafer 定位為最快的 serverless GLM route，因此這條訊息比普通「已支援某模型」的公告更像一條具體的 managed-access path。

Type: Integration | Date: 2026-07-16

<a id="case-225"></a>
### Case 225: [TogetherLink Codex Harness 橋接](https://x.com/nutlope/status/2077432463685554558) (by [@nutlope](https://x.com/nutlope))

**如果你想把 GLM-5.2 跑進現有 coding-agent CLI，可以看這個案例，因為 nutlope 說 TogetherLink 是一個開源 CLI，能讓 Codex 和 Claude Code 直接呼叫 GLM 5.2 這類 open model。**

這則發布把 TogetherLink 定位成一層 bridge：開發者可以繼續使用自己熟悉的 coding harness，同時把底層模型切到 open-weight stack。貼文明確點名支援 Codex 和 Claude Code，並把專案描述成 open source，因此它不是泛泛的可用性消息，而是一條很具體的接入路徑，適合那些想試 GLM-5.2、但不想放棄既有 terminal workflow 的團隊。

Type: Integration | Date: 2026-07-15

---
<a id="case-224"></a>
### Case 224: [Vorflux 開放模型 Harness 路由](https://x.com/vorfluxai/status/2077449967711760587) (by [@vorfluxai](https://x.com/vorfluxai))

**如果你想在不寫自訂 glue code 的前提下，把 GLM-5.2 接進完整 agent session，可以看這個案例，因為 vorfluxai 說它的 Open Model Harness 會把 GLM 5.2 分配給 design、build 和 simplify 步驟，同時保留 Vorflux 其餘流程不變。**

vorfluxai 說，這套 harness 提供了一個下拉切換，可以把整段 session 切到 open models，同時保留 Vorflux 原本的 planning、subagents、pull requests 和 testing 流程。已公開的路由表裡，DeepSeek V4 Pro 負責 main、plan 和 review，DeepSeek V4 Flash 負責 explore，GLM 5.2 負責 design、build 和 simplify，Kimi 2.7 Code 負責 debug 和 testing；這讓它成為一條非常具體的多模型 agent 編排模式，而不是普通的「已支援」公告。

Type: Integration | Date: 2026-07-15

---
<a id="case-170"></a>
### Case 170: [NVIDIA 免費 API 即插即用訪問](https://x.com/hqmank/status/2072855265612030010) (by [@hqmank](https://x.com/hqmank))
...
<a id="case-169"></a>
### Case 169: [Free Workers AI 編碼代理路線](https://x.com/ClaudeCode_UT/status/2072881775408456066) (by [@ClaudeCode_UT](https://x.com/ClaudeCode_UT))
...
**如果你想在 agent tooling 裡測試一條託管式且回應很快的 GLM-5.2 接入路徑，可以看這個案例，因為 QCXINT_ 表示，Databricks AI Gateway 提供了組織專屬 base URL 和 token 流程，暴露出一條速度非常快、看起來支援 1M context 的 GLM 5.2 路由，但後端身分仍未確認。**

QCXINT_ 給出了一套很具體的設定流程：建立 Databricks workspace，打開 User Settings，建立帶 AI Gateway scope 的 access token，複製組織專屬的 AI Gateway base URL，然後從 OpenClaw 或 Hermes 這類 agent tools 呼叫暴露出來的 endpoint。貼文說，目前測試裡只看到了 GLM-5.2，這條路由速度快得有些異常，而且看起來支援最高 1M context；但作者也明確提醒，他還沒有確認 gateway 背後是否真的是它所宣稱的那個精確 GLM-5.2 模型。
...
<a id="case-208"></a>
### Case 208: [打開分子檢視器代理堆疊](https://x.com/MaziyarPanahi/status/2075913552854933869) (by [@MaziyarPanahi](https://x.com/MaziyarPanahi))
...
---
<a id="case-204"></a>
### Case 204: [困惑顧問 WANDR 成本基準](https://x.com/perplexity_ai/status/2075229779716973030) (by [@perplexity_ai](https://x.com/perplexity_ai))
...
<a id="case-203"></a>
### Case 203: [同事開放工件路由](https://x.com/coworkerapp/status/2075233366266310905) (by [@coworkerapp](https://x.com/coworkerapp))
...
<a id="case-201"></a>
### Case 201: [DotCode 上下文上傳工作流程](https://x.com/stagedhappen/status/2074775356867789241) (by [@stagedhappen](https://x.com/stagedhappen))
...
**如果你想在 H200 上 benchmark GLM-5.2 的託管 serving economics，可以看這個案例，因為 RedHat_AI 表示，llm-d 的 Wide EP 加 prefix-cache 路由，讓一條 700B+ 的 GLM-5.2 路線實現了超過 90% 的 cache reuse、低於 3 秒的 TTFT，以及每百萬輸出 token 約 2 美元的成本。**

Red Hat AI 指向 robertshaw21 在 vLLM Office Hours 上的拆解，展示一條運行在 H200 上的 700B+ GLM-5.2 部署。貼文把這條路由超過 90% 的 cache reuse 和低於 3 秒的 TTFT 歸因於 llm-d Wide EP 與 prefix-cache-aware routing，同時還給出了約每百萬輸出 token 2 美元、而託管 API 為 4.40 美元的對比。這使它成為一條很具體的 production-serving 參考，適合拿來比較自管路由 stack 與直接使用託管模型的成本。
...
<a id="case-209"></a>
### Case 209: [Colibri 25GB RAM 稀疏串流媒體](https://x.com/techNmak/status/2075872446197158361) (by [@techNmak](https://x.com/techNmak))
...
---
<a id="case-206"></a>
### Case 206: [SGLang NVFP4 生產吞吐量](https://x.com/sgl_project/status/2075721488456654861) (by [@sgl_project](https://x.com/sgl_project))
...
---
<a id="case-198"></a>
### Case 198: [Dahl 100M 免費 GLM 端點](https://x.com/pengsonal/status/2074908680106180669) (by [@pengsonal](https://x.com/pengsonal))
...
---
<a id="case-195"></a>
### Case 195: [NVIDIA 免費端點 GLM 設定](https://x.com/undefinedKi/status/2074491917333655948) (by [@undefinedKi](https://x.com/undefinedKi))
...
---
<a id="case-179"></a>
### Case 179: [一鍵26款API接入](https://x.com/Alan_Earn/status/2073663239028924680) (by [@Alan_Earn](https://x.com/Alan_Earn))
...
---
<a id="case-176"></a>
### Case 176: [NVIDIA NIM OpenCode 思維設置](https://x.com/Dracoshowumore/status/2073384581256929717) (by [@Dracoshowumore](https://x.com/Dracoshowumore))
...
<a id="case-165"></a>
### Case 165: [透過行動代理控制啟動 ZCode](https://x.com/Digiato/status/2072663459850829985) (by [@Digiato](https://x.com/Digiato))
...
<a id="case-160"></a>
### Case 160: [OpenWiki 自動維護代理文檔](https://x.com/LangChain/status/2072745455788933321) (by [@LangChain](https://x.com/LangChain))
...
<a id="case-152"></a>
### Case 152: [透過 FireConnect 鑄造 PTU](https://x.com/FireworksAI_HQ/status/2072407689964183867) (by [@FireworksAI_HQ](https://x.com/FireworksAI_HQ))
...
<a id="case-147"></a>
### Case 147: [Braintrust GLM 評估工作台](https://x.com/ankrgyl/status/2072042305108722040) (by [@ankrgyl](https://x.com/ankrgyl))
...
<a id="case-141"></a>
### Case 141: [ClinePass 開放權重模型統一訂閱](https://x.com/iam_elias1/status/2071655509611151674) (by [@iam_elias1](https://x.com/iam_elias1))
...
<a id="case-137"></a>
### Case 137: [供 Coding Agents 使用的免費 GLM API 服務](https://x.com/mcwangcn/status/2071261128575897901) (by [@mcwangcn](https://x.com/mcwangcn))
...
<a id="case-32"></a>
### Case 32: [奧拉馬雲端可用性](https://x.com/ollama/status/2066949797316350361) (by [@ollama](https://x.com/ollama))
...
<a id="case-33"></a>
### Case 33: [OpenRouter One API 呼叫存取](https://x.com/OpenRouter/status/2066941552208056561) (by [@OpenRouter](https://x.com/OpenRouter))
...
<a id="case-35"></a>
### Case 35: [透過 Baseten 獲得概念可用性](https://x.com/NotionHQ/status/2066963258985320550) (by [@NotionHQ](https://x.com/NotionHQ))
...
<a id="case-36"></a>
### Case 36: [煙火零日服務](https://x.com/FireworksAI_HQ/status/2067007200426680509) (by [@FireworksAI_HQ](https://x.com/FireworksAI_HQ))
...
<a id="case-37"></a>
### Case 37: [谷歌雲模型花園鏈接](https://x.com/CarolGLMs/status/2067127223216419088) (by [@CarolGLMs](https://x.com/CarolGLMs))
...
<a id="case-38"></a>
### Case 38: [威尼斯隱私模式](https://x.com/AskVenice/status/2066990725439361251) (by [@AskVenice](https://x.com/AskVenice))
...
<a id="case-39"></a>
### Case 39: [命令代碼可用性](https://x.com/CommandCodeAI/status/2066950478194503990) (by [@CommandCodeAI](https://x.com/CommandCodeAI))
...
<a id="case-40"></a>
### Case 40: [來自 Nous Portal 的赫爾墨斯特工](https://x.com/Teknium/status/2066954081575592282) (by [@Teknium](https://x.com/Teknium))
...
<a id="case-41"></a>
### Case 41: [io.net 零日啟動合作夥伴](https://x.com/ionet/status/2067140436095803763) (by [@ionet](https://x.com/ionet))
...
<a id="case-42"></a>
### Case 42: [模組化雲端零日服務](https://x.com/clattner_llvm/status/2066974950293147950) (by [@clattner_llvm](https://x.com/clattner_llvm))
...
<a id="case-61"></a>
### Case 61: [透過 OpenRouter 設定遊標](https://x.com/agentnative_/status/2068148384846746107) (by [@agentnative_](https://x.com/agentnative_))
...
<a id="case-63"></a>
### Case 63: [放大視覺設計的代理眼睛](https://x.com/beyang/status/2068087124818317374) (by [@beyang](https://x.com/beyang))
...
<a id="case-69"></a>
### Case 69: [Baseten 更快的一百萬情境服務](https://x.com/alphatozeta8148/status/2067852860499562821) (by [@alphatozeta8148](https://x.com/alphatozeta8148))
...
<a id="case-74"></a>
### Case 74: [面向網頁設計的 Browser Use QA 子代理](https://x.com/browser_use/status/2068405699340853541) (by [@browser_use](https://x.com/browser_use))
...
<a id="case-79"></a>
### Case 79: [ZCode 官方 IDE 每日免費額度](https://x.com/Alan_Earn/status/2068223665268006924) (by [@Alan_Earn](https://x.com/Alan_Earn))
...
<a id="case-83"></a>
### Case 83: [透過 Fireworks 設定遊標](https://x.com/skirano/status/2068777440986513647) (by [@skirano](https://x.com/skirano))
...
<a id="case-84"></a>
### Case 84: [VulcanBench ZAI 提供者支持](https://x.com/vulcanbench/status/2068724843856707676) (by [@vulcanbench](https://x.com/vulcanbench))
...
<a id="case-85"></a>
### Case 85: [OpenCode 高/最大推理變體](https://x.com/OpenCodeLog/status/2068487086576156705) (by [@OpenCodeLog](https://x.com/OpenCodeLog))
...
<a id="case-86"></a>
### Case 86: [Z.ai編碼端點選擇](https://x.com/ivanfioravanti/status/2068574700721082400) (by [@ivanfioravanti](https://x.com/ivanfioravanti))
...
<a id="case-87"></a>
### Case 87: [ZenMux 免費 GLM-5.2 API 設定](https://x.com/0x_kaize/status/2068676992782811607) (by [@0x_kaize](https://x.com/0x_kaize))
...
<a id="case-93"></a>
### Case 93: [本體 ncode GLM 預設值](https://x.com/_xjdr/status/2069030608727408993) (by [@_xjdr](https://x.com/_xjdr))
...
<a id="case-95"></a>
### Case 95: [克勞德代碼通過 Baseten](https://x.com/thealexker/status/2069163621469335757) (by [@thealexker](https://x.com/thealexker))
...
<a id="case-96"></a>
### Case 96: [Deepsec Pi 代理預設值](https://x.com/cramforce/status/2069057402524082622) (by [@cramforce](https://x.com/cramforce))
...
<a id="case-101"></a>
### Case 101: [Baseten + Droid 安全帶快速入門](https://x.com/RayFernando1337/status/2069523126384525639) (by [@RayFernando1337](https://x.com/RayFernando1337))
...
<a id="case-104"></a>
### Case 104: [OpenAI 相容的 GLM API 工作流程](https://x.com/Marktechpost/status/2069308807570915500) (by [@Marktechpost](https://x.com/Marktechpost))
...
<a id="case-105"></a>
### Case 105: [Perplexity Agent API 搜尋沙箱](https://x.com/perplexitydevs/status/2069252848647606584) (by [@perplexitydevs](https://x.com/perplexitydevs))
...
<a id="case-109"></a>
### Case 109: [Baseten 的 280 TPS GLM API 服務](https://x.com/aqaderb/status/2069220126272999501) (by [@aqaderb](https://x.com/aqaderb))
...
<a id="case-128"></a>
### Case 128: [Cloudflare Workers AI OpenCode 設定](https://x.com/RoundtableSpace/status/2070820686243959276) (by [@RoundtableSpace](https://x.com/RoundtableSpace))
...
<a id="case-129"></a>
### Case 129: [Puter.js 零設定瀏覽器客戶端](https://x.com/FareaNFts/status/2070848321212792945) (by [@FareaNFts](https://x.com/FareaNFts))
...
<a id="case-130"></a>
### Case 130: [SiliconFlow 統一端點接入](https://x.com/FareaNFts/status/2070900056736379288) (by [@FareaNFts](https://x.com/FareaNFts))
...
<a id="case-125"></a>
### Case 125: [DigitalOcean Inference Engine 上線](https://x.com/digitalocean/status/2070177703911719301) (by [@digitalocean](https://x.com/digitalocean))
...
<a id="case-115"></a>
### Case 115: [Command Code Fast 120-250 Tok/S 檔位](https://x.com/CommandCodeAI/status/2069891896881857016) (by [@CommandCodeAI](https://x.com/CommandCodeAI))
...
<a id="case-116"></a>
### Case 116: [Vercel AI 閘道快速 GLM-5.2 API](https://x.com/wafer_ai/status/2069869501190152528) (by [@wafer_ai](https://x.com/wafer_ai))
...
---
<a id="case-254"></a>
### Case 254: [8x GB10 860K 稀疏服務](https://x.com/light_foundry/status/2079158726658060652) (by [@light_foundry](https://x.com/light_foundry))

**如果你想 benchmark 基於 GB10 級叢集的長上下文本地 GLM-5.2 服務，可以看這個案例，因為 light_foundry 說一套 8x GB10 配置，配合 sparse indexer 和 Int4-Int8Mix 權重，在 4K 深度下做到了 1,101 tok/s prefill，最高可服務到 860K tokens，並且在 845K context 仍然能找回 needle。**

light_foundry 描述的是一套把 8x GB10 叢集遷移到統一 base 加 `b12x` sparse indexer 的配置，用來運行帶 fp8 sparse-MLA KV 和 MTP 的 GLM-5.2 Int4-Int8Mix。貼文給出的數字包括：4K 深度下 prefill 從 880 提高到 1,101 tok/s，77K / 231K / 395K 深度下的 prefill 分別為 1,052 / 940 / 838 tok/s，單流 decode 中位數為 42.1 tok/s，以及一個可支援 860K 最大長度的 903K-token KV 池，並且在約 845K 深度下仍能以 10.9 tok/s 解碼。貼文還特別指出兩個很具體的運維陷阱：冷啟動時的 inductor cache 編譯看起來像 shared-memory hang，以及 GPU memory fragmentation 會在多次重啟後壓縮可用 KV。

Type: Evaluation | Date: 2026-07-20

---
<a id="case-253"></a>
### Case 253: [混合 3/4/8-Bit 本地權衡](https://x.com/0xSero/status/2079184434188685668) (by [@0xSero](https://x.com/0xSero))

**如果你想在退回 BF16 之前，先估算一條激進量化的本地 GLM-5.2 路線值不值得走，可以看這個案例，因為 0xSero 說一套混合 3/4/8-bit 配置，仍然在 Terminal-Bench 2.1 上做到 70.8%，在 GPQA Diamond 上做到 88.89%，在 4x RTX 6000 上約 82 tok/s，並把單任務成本壓到約 0.22 美分。**

這則貼文把一套量化版 GLM-5.2-Hybrid 的評測與服務數據打包放在一起：Terminal-Bench 2.1 成績為 70.8%，個別運行最高到 77.8%；GPQA Diamond 為 88.89%；在 4x RTX 6000 GPU 上約 1,200 tok/s prefill、約 82 tok/s decode，context 大約 340K，單任務成本約 0.22 美分。貼文還聲稱，它和 BF16 相比，在 Terminal-Bench 上只差 3 到 8 個點，在 GPQA 上只差 0.15，因此對於要判斷為了降低本地 GLM 服務成本與記憶體占用，最多能接受多少品質損失的團隊來說，這是一個非常具體的權衡參考。
...
**如果你想用實際收據而不是牌價來校驗 GLM-5.2 的真實經濟性，可以看這個案例，因為 AstridWiegner 說一張 ZenMux Token Receipt 顯示處理量超過 2.49 億 token，原始成本是 105.81 美元，但最終總額是 0 美元。**

AstridWiegner 表示，一張 ZenMux Token Receipt 記錄了超過 2.49 億個 GLM 5.2 token，原始成本顯示為 105.81 美元，但最後總額降到了 0.00 美元。貼文把這類收據視為比單純 benchmark 分數更有用的比較介面，因為它把輸出品質與工作負載規模，直接對應到固定預算實際能買到多少工作量。

Type: Evaluation | Date: 2026-07-17

---
<a id="case-241"></a>
### Case 241: [Zro Pro 3 億 GLM 試用](https://x.com/AndarkFomo/status/2078092015368368574) (by [@AndarkFomo](https://x.com/AndarkFomo))

**如果你想用有限預算測試私有託管的 GLM-5.2 agent 工作，可以看這個案例，因為 AndarkFomo 說一個 Zro Pro 活動大約能用 1 美元解鎖 3 億 GLM-5.2 token，並附帶 OpenAI 相容存取、EU infra 與 zero-retention 定位。**

AndarkFomo 說，Product Hunt 優惠碼 `PRODUCTHUNT` 會給前 100 位使用者一個月免費的 Zro Pro，而這個方案平常月費是 20 美元；同時也有人在結帳時只被觸發了 1 美元的信用卡預授權。貼文表示，這個方案透過 OpenAI 相容 API 在 EU infra 上提供 private open-model inference 給 coding agent 使用，而且不會拿 prompts 訓練模型；它也補充了實際 caveat：3 億 token 更像是預期可用量，而不是永久硬上限，名額有限，且這條路線上的 GLM-5.2 context 為 350K。
...
**如果你想估算桌面級 GLM-5.2 部署的規模，可以看這個案例，因為 TheAhmadOsman 說 GLM 5.2 NVFP4 在 DGX Station 上以 256K context 跑出了約 3,000 tok/s 的 prefill 與 32 tok/s 的 decode。**

TheAhmadOsman 表示，這次運行使用的是帶 FP8 KV cache 的 GLM 5.2 NVFP4，部署在一台 DGX Station 上，在 256K context length 下大約達到 3,000 tokens per second 的 prefill，以及 32 tokens per second 的 decode。貼文也把取捨講得很直接：目前還不支援併發請求，但對於本地長上下文方案來說，單桌面的吞吐表現已經相當紮實。
...
**如果你想快速掌握一條帶公開定價的 hosted GLM-5.2 接入路徑，可以看這個案例，因為 JatevoId 說其平台上 GLM 5.2 的 input 價格是 $1.40 / 1M、output 價格是 $4.40 / 1M，符合條件的 JTVO holder 還能拿到 50% 折扣。**

JatevoId 說，這次 rollout 還帶有面向 holder 的分層 free-compute 配額，並在標準 per-token 定價之外公開給出 50% 的 discount policy。明確的 input / output 價格，讓它即使在折扣適用條件依賴平台規則的情況下，依然是一條很具體的 access note，而不是模糊的 launch 貼文。

Type: Integration | Date: 2026-07-16

---
<a id="case-233"></a>
### Case 233: [MI325x 上低於 0.1 美分的 GLM 服務](https://x.com/picocreator/status/2077817481381728268) (by [@picocreator](https://x.com/picocreator))

**如果你想評估 AMD 硬體上的 self-hosted GLM-5.2 inference 成本，可以看這個案例，因為 picocreator 說 4xMI325x 配置把 GLM 5.2 跑到了 1,482 tok/s，成本低於每百萬 token 0.10 美元。**

picocreator 說，這條 route 在四張 MI325x GPU 上實現了 1,482 tokens per second，成本大約是 B300s 的三分之一、Opus 的十分之一。它強調的不是 API 標價，而是專用硬體上實際跑出的 GLM capacity 成本，因此對於在評估 self-hosting economics 的團隊來說，這是一個很有參考價值的 checkpoint。

Type: Demo | Date: 2026-07-16

---
<a id="case-226"></a>
### Case 226: [Bonsai Mac Studio 病歷分流](https://x.com/MaziyarPanahi/status/2077362554805117132) (by [@MaziyarPanahi](https://x.com/MaziyarPanahi))

**如果你想讓一份很長的臨床病歷留在本地，同時讓 GLM-5.2 在其上做推理，可以看這個案例，因為 MaziyarPanahi 說 GLM 5.2 透過 Mac Studio 上的 Bonsai 27B 分流一份三年期病歷，並找出了埋在 17 個月前的造影風險問題。**

MaziyarPanahi 說，292 次就診紀錄都留在 Mac Studio 上的 Bonsai 27B 裡，透過 llama.cpp、Metal、ternary weights，以及大約 7.2GB 的本地模型儲存來處理；同時只允許 GLM-5.2 問三個問題，最後就定位到了 eGFR 39 條件下 metformin 與含碘顯影劑並用的風險。貼文把這套架構明確描述為 privacy-preserving：病歷從未離開機器，orchestrator 也從不接觸原始病人資料，因此這是一條很具體的本地醫療 workflow，而不是泛泛的模型背書。

Type: Demo | Date: 2026-07-15

---
<a id="case-191"></a>
### Case 191: [Hermes 建立的 LiteLLM 本地實驗室](https://x.com/ivanfioravanti/status/2074609005272375329) (by [@ivanfioravanti](https://x.com/ivanfioravanti))
...
---
<a id="case-187"></a>
### Case 187: [雙 M5 Max 離線無人機模擬器](https://x.com/XavierLocalAI/status/2073938465121833452) (by [@XavierLocalAI](https://x.com/XavierLocalAI))
...
---
<a id="case-186"></a>
### Case 186: [5x DGX Spark 生產線束](https://x.com/thatcofffeeguy/status/2074245620207058981) (by [@thatcofffeeguy](https://x.com/thatcofffeeguy))
...
---
<a id="case-183"></a>
### Case 183: [M3 Ultra ds4-eval Q4 檢查點](https://x.com/ivanfioravanti/status/2073742792044446194) (by [@ivanfioravanti](https://x.com/ivanfioravanti))
...
---
<a id="case-182"></a>
### Case 182: [4x RTX PRO 6000 建置指南](https://x.com/QingQ77/status/2073589933567094981) (by [@QingQ77](https://x.com/QingQ77))
...
---
<a id="case-181"></a>
### Case 181: [4x DGX Spark QuantTrio 運行](https://x.com/Tech2Wild/status/2073637530960826787) (by [@Tech2Wild](https://x.com/Tech2Wild))
...
---
<a id="case-177"></a>
### Case 177: [單一 M3 Ultra 4 位元影片運行](https://x.com/ivanfioravanti/status/2073502277449486460) (by [@ivanfioravanti](https://x.com/ivanfioravanti))
...
<a id="case-173"></a>
### Case 173: [AMD MI355X 2626 Tok/s 節點服務](https://x.com/wafer_ai/status/2073155792182907085) (by [@wafer_ai](https://x.com/wafer_ai))
...
<a id="case-172"></a>
### Case 172: [Vercel 閘道 287 Tok/s 無伺服器](https://x.com/wafer_ai/status/2072861749104288074) (by [@wafer_ai](https://x.com/wafer_ai))
...
<a id="case-171"></a>
### Case 171: [一鍵 RTX PRO 6000 部署](https://x.com/XciD_/status/2073035324272328733) (by [@XciD_](https://x.com/XciD_))
...
<a id="case-164"></a>
### Case 164: [中國堆疊中的代理路由交換](https://x.com/0xluffy_eth/status/2072548580183900430) (by [@0xluffy_eth](https://x.com/0xluffy_eth))
...
<a id="case-156"></a>
### Case 156: [744B 本地硬體層](https://x.com/devjuninho/status/2072151237840007399) (by [@devjuninho](https://x.com/devjuninho))
...
<a id="case-151"></a>
### Case 151: [本地 NVFP4 Rust 連接埠速度為 140 Tok/s](https://x.com/mov_axbx/status/2071839859723882771) (by [@mov_axbx](https://x.com/mov_axbx))
...
<a id="case-140"></a>
### Case 140: [B300 x2 Agent 主導雙棧 Bring-Up](https://x.com/TheValueist/status/2071261052080148607) (by [@TheValueist](https://x.com/TheValueist))
...
<a id="case-139"></a>
### Case 139: [oMLX M3 Ultra Prefill 加速](https://x.com/jundotkim/status/2071287585297887510) (by [@jundotkim](https://x.com/jundotkim))
...
<a id="case-138"></a>
### Case 138: [註冊即送 2000 萬 Token 額度爆發](https://x.com/Bitbro4crypto/status/2071150218872283262) (by [@Bitbro4crypto](https://x.com/Bitbro4crypto))
...
<a id="case-131"></a>
### Case 131: [4x DGX Spark 本地 GLM 操作手冊](https://x.com/TraffAlex/status/2071020631072616698) (by [@TraffAlex](https://x.com/TraffAlex))
...
<a id="case-43"></a>
### Case 43: [輸出代幣成本比較](https://x.com/Hesamation/status/2066999955638673891) (by [@Hesamation](https://x.com/Hesamation))
...
<a id="case-44"></a>
### Case 44: [本地近前沿硬體投資報酬率](https://x.com/Jeyffre/status/2067132023576354818) (by [@Jeyffre](https://x.com/Jeyffre))
...
<a id="case-45"></a>
### Case 45: [兩個 Mac 工作室上的 MLX](https://x.com/pcuenq/status/2066967665726337219) (by [@pcuenq](https://x.com/pcuenq))
...
<a id="case-46"></a>
### Case 46: [H100 每月本地部署索賠](https://x.com/ai_xiaomu/status/2066181367340429446) (by [@ai_xiaomu](https://x.com/ai_xiaomu))
...
<a id="case-47"></a>
### Case 47: [每日積分和克勞德更換索賠](https://x.com/RoundtableSpace/status/2067076011770986715) (by [@RoundtableSpace](https://x.com/RoundtableSpace))
...
<a id="case-48"></a>
### Case 48: [免費 ZCode 令牌窗口](https://x.com/0xSero/status/2066772591319077208) (by [@0xSero](https://x.com/0xSero))
...
<a id="case-49"></a>
### Case 49: [ZenMux 免費週優惠](https://x.com/JZiyue_/status/2067114018079412723) (by [@JZiyue_](https://x.com/JZiyue_))
...
<a id="case-50"></a>
### Case 50: [crofAI 每代幣定價](https://x.com/nahcrof/status/2067166596523765781) (by [@nahcrof](https://x.com/nahcrof))
...
<a id="case-51"></a>
### Case 51: [API價差對比](https://x.com/scaling01/status/2066952626386714906) (by [@scaling01](https://x.com/scaling01))
...
<a id="case-88"></a>
### Case 88: [兩台 M3 Ultra MLX 分散式運行](https://x.com/ivanfioravanti/status/2068781499206574576) (by [@ivanfioravanti](https://x.com/ivanfioravanti))
...
<a id="case-89"></a>
### Case 89: [ZCode 乘數貫穿 9 月份](https://x.com/buildwithhassan/status/2068534544177791376) (by [@buildwithhassan](https://x.com/buildwithhassan))
...
<a id="case-98"></a>
### Case 98: [Mac Studio API 投資報酬率現實檢查](https://x.com/karminski3/status/2068974488973447524) (by [@karminski3](https://x.com/karminski3))
...
<a id="case-106"></a>
### Case 106: [LiteLLM 本地中斷保存](https://x.com/CardilloSamuel/status/2069431311463236078) (by [@CardilloSamuel](https://x.com/CardilloSamuel))
...
<a id="case-107"></a>
### Case 107: [個人與團隊本地投資報酬率](https://x.com/yuhasbeentaken/status/2069337770493919414) (by [@yuhasbeentaken](https://x.com/yuhasbeentaken))
...
<a id="case-118"></a>
### Case 118: [2x RTX PRO 6000 Blackwell 本地 Crackme 解題](https://x.com/CardilloSamuel/status/2069887782508753302) (by [@CardilloSamuel](https://x.com/CardilloSamuel))
...
**如果你想測試 GLM-5.2 在矛盾輸入處理上的表現，而不是只看 clean-pass benchmark 數字，可以看這個案例，因為 cyrilXBT 表示，一次 KV-cache debugger 的正面對比顯示，GLM 在乾淨配置上和其他模型一樣答對，但會悄悄漏掉一個壞變數，並在沒有任何警告的情況下給出一個 2.667x 錯誤的 preset。**

cyrilXBT 描述的不是氛圍式測試，而是一個具體 benchmark artifact：單檔 HTML 的 KV-cache debugger，帶精確公式、五個 preset、一個 testing API，以及覆蓋 GPT-5.6 Sol、Fable 5、Grok 4.5 和 GLM 5.2 的 11 項客觀正確性檢查。貼文說，四個模型在乾淨配置上都答對了，但 GLM 5.2 漏掉了一條矛盾路徑，讓一個 preset 在沒有跳出 warning 的情況下錯到 2.667x，這使它成為一條對工具型 UI build 很實用的限制訊號：如果你需要防禦式校驗，就不能只看表面分數。
...
<a id="case-205"></a>
### Case 205: [靜態 HTML 重寫執行器未命中](https://x.com/petruknisme/status/2075092910182387759) (by [@petruknisme](https://x.com/petruknisme))
...
<a id="case-197"></a>
### Case 197: [Composio 47-任務代理差距](https://x.com/composio/status/2074908761970393265) (by [@composio](https://x.com/composio))
...
---
<a id="case-163"></a>
### Case 163: [初步網路研究平等](https://x.com/Irregular/status/2072682835798831168) (by [@Irregular](https://x.com/Irregular))
...
<a id="case-157"></a>
### Case 157: [OpenRouter 削減開支技能重寫](https://x.com/Rahul_J_Mathur/status/2072279035493900395) (by [@Rahul_J_Mathur](https://x.com/Rahul_J_Mathur))
...
<a id="case-149"></a>
### Case 149: [AA 冗長和推理權衡](https://x.com/ArtificialAnlys/status/2072022576394821859) (by [@ArtificialAnlys](https://x.com/ArtificialAnlys))
...
<a id="case-134"></a>
### Case 134: [Semgrep IDOR 狹義勝出提醒](https://x.com/leploutos/status/2071121981551047039) (by [@leploutos](https://x.com/leploutos))
...
<a id="case-133"></a>
### Case 133: [PrinzBench 領域錯配提醒](https://x.com/yuhasbeentaken/status/2070928066797351300) (by [@yuhasbeentaken](https://x.com/yuhasbeentaken))
...
<a id="case-52"></a>
### Case 52: [無視力警告](https://x.com/sawyerhood/status/2067061246705426923) (by [@sawyerhood](https://x.com/sawyerhood))
...
<a id="case-53"></a>
### Case 53: [現實世界的代理差距警告](https://x.com/ml_angelopoulos/status/2067013545431269405) (by [@ml_angelopoulos](https://x.com/ml_angelopoulos))
...
<a id="case-54"></a>
### Case 54: [安全護欄關注](https://x.com/VittoStack/status/2066984051840606436) (by [@VittoStack](https://x.com/VittoStack))
...
<a id="case-55"></a>
### Case 55: [基準方法論批評](https://x.com/josepha_mayo/status/2066951013337112661) (by [@josepha_mayo](https://x.com/josepha_mayo))
...
<a id="case-56"></a>
### Case 56: [尖峰時段延遲問題](https://x.com/k_matsumaru/status/2067023197166559621) (by [@k_matsumaru](https://x.com/k_matsumaru))
...
<a id="case-57"></a>
### Case 57: [FutureSim 無改善結果](https://x.com/nikhilchandak29/status/2066970561511657913) (by [@nikhilchandak29](https://x.com/nikhilchandak29))
...
<a id="case-58"></a>
### Case 58: [發射準備批評](https://x.com/bridgemindai/status/2065770088821600512) (by [@bridgemindai](https://x.com/bridgemindai))
...
<a id="case-59"></a>
### Case 59: [編碼計劃價格上漲](https://x.com/bridgemindai/status/2065799843658793092) (by [@bridgemindai](https://x.com/bridgemindai))
...
<a id="case-67"></a>
### Case 67: [短時間並行工作與長代理運行](https://x.com/thekuchh/status/2068010332501479865) (by [@thekuchh](https://x.com/thekuchh))
...
<a id="case-73"></a>
### Case 73: [程式碼審查與偏見檢查](https://x.com/wongmjane/status/2068424945663893936) (by [@wongmjane](https://x.com/wongmjane))
...
<a id="case-75"></a>
### Case 75: [高難推理計費異常](https://x.com/s_batzoglou/status/2068297051247350154) (by [@s_batzoglou](https://x.com/s_batzoglou))
...
<a id="case-103"></a>
### Case 103: [HalluHard 多圈幻覺訊號](https://x.com/dyfan22/status/2069335764295438532) (by [@dyfan22](https://x.com/dyfan22))
...
<a id="case-108"></a>
### Case 108: [開放式安全緊急警示](https://x.com/joshua_saxe/status/2069289170107842572) (by [@joshua_saxe](https://x.com/joshua_saxe))
...
<a id="case-126"></a>
### Case 126: [Rust 錯誤修復通過但回合數高 7 倍](https://x.com/BohuTANG/status/2069979942608425364) (by [@BohuTANG](https://x.com/BohuTANG))
...
<a id="case-114"></a>
### Case 114: [Braintrust 模型替換成本警示](https://x.com/ankrgyl/status/2069869387549446597) (by [@ankrgyl](https://x.com/ankrgyl))
...
- [閱讀 GLM-5.2 API 文件](https://docs.evolink.ai/en/api-manual/language-series/glm-5.2/glm-5.2-api?utm_source=github&utm_medium=docs&utm_campaign=awesome-glm-5.2-usecases&utm_content=first_run) - 目前模型已驗證的首次執行入口。

本指南不聲稱存在已驗證且可安裝的 GLM-5.2 skill 儲存庫；請使用上方 API 文件。
