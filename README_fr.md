...
- **258 cas GLM-5.2 sélectionnés** provenant de créateurs publics, équipes de benchmark, développeurs d’outils, fournisseurs et utilisateurs de terrain.
...
| [💻 Agents de code et flux de travail à long contexte](#coding-agents-long-context-workflows) | Case 13-22, 62, 65, 66, 77, 80, 91, 102, 117, 119, 122, 127, 135-136, 142-143, 145, 148, 150, 153, 155, 168, 174, 180, 194, 210-212, 228, 236-237, 243, 255, 257 |
...
| [🔌 Intégrations fournisseurs et outils](#provider-tool-integrations) | Case 31-42, 61, 63, 69, 74, 79, 83-87, 93, 95-96, 101, 104-105, 109, 115-116, 124-125, 128-130, 137, 141, 147, 152, 160, 165, 169-170, 176, 179, 185, 193, 195, 198, 201, 203-204, 208, 214, 219-220, 224-225, 230-232, 238-239, 256, 258 |
| [💸 Coût, prix et déploiement local](#cost-pricing-local-deployment) | Case 43-51, 64, 68, 88-89, 97-98, 106-107, 112, 118, 131, 138-140, 151, 156, 164, 166, 171-173, 177, 181-183, 186-187, 191, 206, 209, 215, 221, 226, 233-234, 240-246, 249, 251, 253-254 |
...
| [Case 243: Service hybride Hermes à parité API](#case-243) | Utilisez ce cas pour valider un coding agent GLM-5.2 auto-hébergé face à la route officielle, car dangerm00se dit qu'un hybride Hermes plus GLM-5.2 sur 4x RTX 6000 PCIe a correspondu à 59 tâches sur 60 de l'API officielle tout en livrant 3,149 tok/s de prefill, 0.37 s de warm TTFT et 35.9 tok/s de decode. | Evaluation |
| [Case 237: Agent GLM LM Studio Bionic](#case-237) | Utilisez ce cas pour évaluer un coding agent GLM-5.2 local-first, car chenzeling4 dit que LM Studio Bionic associe GLM 5.2 à des sandboxes documentaires locales, des diffs de code inline, des checkpoints de rollback et une transcription vocale on-device. | Integration |
| [Case 236: Avantage qualité web dev de Claude Code](#case-236) | Utilisez ce cas pour comparer la qualité web dev au premier passage plutôt que la seule vitesse d’exécution, car Lumenix0 dit que GLM 5.2 dans Claude Code a battu GPT 5.5 dans Codex sur la qualité du design et la complétude fonctionnelle sur trois tâches réelles. | Evaluation |
...
| [Case 239: Fenêtre gratuite d’API GLM chez TokenRouter](#case-239) | Utilisez ce cas pour saisir une route d’API GLM-5.2 gratuite à court terme, car meliasiih dit que TokenRouter offre un accès gratuit jusqu’au 25 juillet 2026 avec une inscription simple, un flux de clé API et une base URL publiée. | Tutorial |
| [Case 238: Gateway GLM Wafer sur Agentuity](#case-238) | Utilisez ce cas pour ajouter GLM-5.2 à une stack gateway sur Agentuity, car wafer_ai dit que sa route serverless sert désormais GLM 5.2 sur Agentuity à environ 100 à 250 tok/s avec 1M de contexte sur les paliers standard et Fast. | Integration |
...
| [Case 242: Reçu ZenMux de 249M tokens](#case-242) | Utilisez ce cas pour vérifier l’économie réelle de GLM-5.2 à partir de reçus plutôt que de tarifs catalogue, car AstridWiegner dit qu’un ZenMux Token Receipt montrait plus de 249M tokens traités, un coût initial de 105.81 dollars et un total final de 0 dollar. | Evaluation |
| [Case 241: Essai GLM 300M sur Zro Pro](#case-241) | Utilisez ce cas pour tester un travail agentique GLM-5.2 hébergé en privé avec un budget serré, car AndarkFomo dit qu’une promo Zro Pro peut débloquer environ 300M de tokens GLM-5.2 pour environ 1 dollar avec un accès compatible OpenAI, une infra UE et un positionnement zero-retention. | Tutorial |
| [Case 240: Serving desktop 256K sur DGX Station](#case-240) | Utilisez ce cas pour dimensionner un déploiement GLM-5.2 de classe desktop, car TheAhmadOsman dit que GLM 5.2 NVFP4 a tourné avec 256K de contexte sur une DGX Station avec environ 3,000 tok/s en prefill et 32 tok/s en decode. | Demo |
...
| Cas | Point clé | Type |
|---|---|---|
| [Case 247: Correctif RCE par défaut de ZCode](#case-247) | Utilisez ce cas pour justifier un sandboxing plus strict autour des coding agents GLM-5.2, car weezerOSINT dit qu'une exécution ZCode avec un seul prompt a exécuté du code du dépôt avec une RCE complète par défaut et que le problème a été signalé puis corrigé en version 3.3.6. | Limit |
...
**Utilisez ce cas pour évaluer GLM-5.2 sur des agents de recherche actions cotées, car karinanguyen dit que DiligenceBench a classé GLM 5.2 près du sommet et montré que le harness finance peut rendre les bons modèles à la fois meilleurs et moins chers.**

karinanguyen présente DiligenceBench comme une évaluation à base de rubriques pour la recherche sur les actions cotées et dit que Meta Muse Spark 1.1 a mené le harness finance à 57.4 percent, avec GLM 5.2 juste derrière Sonnet 4.6 et GPT-5.6 Sol. Le post soutient aussi que les outils génériques aident surtout les modèles déjà forts, tandis que les modèles plus faibles ont besoin d’un échafaudage plus spécifique au domaine, et conclut que ce harness finance déplace assez la frontière prix-performance pour que GLM 5.2 ressorte en performance absolue tandis que MiniMax M3 paraît le plus efficace.
...
<a id="case-257"></a>
### Case 257: [Workflow OpenCodex avec changement de modèle](https://x.com/vista8/status/2079239701391675404) (by [@vista8](https://x.com/vista8))

**Utilisez ce cas pour router GLM-5.2 dans une boucle de coding centrée sur Codex au lieu de rester bloqué sur un seul modèle, car vista8 dit qu’OpenCodex permet à un même environnement de basculer entre GLM 5.2, Kimi K3, GPT-5.6 Sol et Grok 4.5 pour le design frontend, le travail backend et la recherche X en direct.**

vista8 dit que Codex restait l’outil de coding le plus utilisé en pratique, mais que le goût frontend demeurait un point faible, d’où le passage vers un projet OpenCodex capable de changer de modèle à la demande. La note de routage est concrète : utiliser Kimi K3 via OAuth pour le design frontend, passer à GPT-5.6 Sol pour le backend, garder Grok 4.5 pour la recherche X et traiter GLM 5.2 comme une autre option interchangeable dans la même surface de coding. Cela fait de ce post un vrai workflow de routeur de modèles pour coding agents, pas un simple avis de classement.

Type: Integration | Date: 2026-07-20

---
<a id="case-255"></a>
### Case 255: [Labo hybride Hermes à 11 agents](https://x.com/MichaelGannotti/status/2079168568478912834) (by [@MichaelGannotti](https://x.com/MichaelGannotti))

**Utilisez ce cas pour structurer un labo multi-agent à rôles autour de GLM-5.2 plutôt qu’un assistant monolithique, car MichaelGannotti dit qu’un setup Hermes à 11 agents répartit dynamiquement les tâches entre DGX Spark, des workstations Ryzen et des modèles cloud dont GLM 5.2 pour le logiciel, la recherche, le marketing et la coordination.**

MichaelGannotti décrit une architecture de labo concrète : un DGX Spark avec 128GB de mémoire unifiée comme serveur local principal d’inférence, une machine Ryzen 9 Halo Strix qui exécute huit agents Hermes pour les rôles d’ingénierie et d’organisation, et une machine Windows Ryzen 7 qui fait tourner trois agents supplémentaires pour le contenu et la pile Microsoft. Dans ce setup, chaque agent choisit selon la tâche entre GPT-5.6, GLM 5.2 et Grok 4.5 dans le cloud, ou des modèles locaux Qwen, Nemotron et Gemma. Le post devient ainsi une vraie référence d’exploitation pour des équipes d’agents hybrides local-cloud, pas une vague promesse de productivité.
...
---
<a id="case-243"></a>
### Case 243: [Service hybride Hermes à parité API](https://x.com/dangerm00se/status/2078369336239313368) (by [@dangerm00se](https://x.com/dangerm00se))

**Utilisez ce cas pour valider un coding agent GLM-5.2 auto-hébergé face à la route officielle, car dangerm00se dit qu'un hybride Hermes plus GLM-5.2 sur 4x RTX 6000 PCIe a correspondu à 59 tâches sur 60 de l'API officielle tout en livrant 3,149 tok/s de prefill, 0.37 s de warm TTFT et 35.9 tok/s de decode.**

dangerm00se présente ce résultat comme un jalon d'autonomie et non comme un simple screenshot de throughput. Le post dit que l'hybride Hermes plus GLM-5.2 a tourné sur 4x RTX 6000 PCIe avec un hôte DDR4, a atteint 3,149 tok/s de prefill avec 0.37 seconde de warm TTFT et 35.9 tok/s de decode, et a rejoint l'API officielle de GLM sur 59 tâches sur 60. Le même post renvoie aussi vers des instructions de setup et un rapport de précision contre la route officielle, ce qui en fait une référence d'agent local concrète plutôt qu'une vantardise vague sur le self-hosting.

Type: Évaluation | Date: 2026-07-18

<a id="case-237"></a>
### Case 237: [Agent GLM LM Studio Bionic](https://x.com/chenzeling4/status/2077967277698515184) (by [@chenzeling4](https://x.com/chenzeling4))

**Utilisez ce cas pour évaluer un coding agent GLM-5.2 local-first, car chenzeling4 dit que LM Studio Bionic associe GLM 5.2 à des sandboxes documentaires locales, des diffs de code inline, des checkpoints de rollback et une transcription vocale on-device.**

chenzeling4 décrit LM Studio Bionic comme un agent autonome construit autour de modèles open-weight, avec un calcul local ou cloud selon la tâche et un positionnement zero-retention dans les deux cas. Le post dit que GLM 5.2 et Kimi K2.7 Code gèrent le coding, que les documents tournent dans une sandbox avec checkpoints de rollback automatiques, que les modifications de code apparaissent sous forme de diffs inline et que la transcription vocale reste on-device via Voxtral. Cela en fait une mise à jour concrète de workflow d’agent local plutôt qu’une simple note de disponibilité de modèle.

Type: Integration | Date: 2026-07-17

---
<a id="case-236"></a>
### Case 236: [Avantage qualité web dev de Claude Code](https://x.com/Lumenix0/status/2078241726897230164) (by [@Lumenix0](https://x.com/Lumenix0))

**Utilisez ce cas pour comparer la qualité web dev au premier passage plutôt que la seule vitesse d’exécution, car Lumenix0 dit que GLM 5.2 dans Claude Code a battu GPT 5.5 dans Codex sur la qualité du design et la complétude fonctionnelle sur trois tâches réelles.**

Lumenix0 résume un face-à-face qui utilisait GLM 5.2 via Claude Code et GPT 5.5 via Codex sur une refonte de site web, une chasse aux bugs React et la construction d’un tableau Kanban. Le post dit que GPT a terminé plus vite sur chaque tâche, mais que GLM a produit la refonte la plus solide, a égalé la correction du bug avec une explication plus claire et a livré le seul tableau Kanban entièrement fonctionnel en one shot avec nommage des tâches, priorités, changements de statut et bouton de reset. La même source dit aussi que les trois tests n’ont consommé que 7 percent du quota hebdomadaire du plan à 16 dollars.
...
<a id="case-258"></a>
### Case 258: [Accès GLM à -50 % via Orbit Provider](https://x.com/orbiteditor/status/2079148011259916552) (by [@orbiteditor](https://x.com/orbiteditor))

**Utilisez ce cas pour tester GLM-5.2 dans Orbit sans gérer des identifiants provider séparés, car Orbit Editor v0.4.0 dit que son nouvel Orbit Provider expose GLM-5.2 directement dans l’éditeur et a été lancé avec une réduction de 50 %.**

L’annonce d’Orbit Editor v0.4.0 introduit une couche provider intégrée pour les modèles open source, avec l’objectif déclaré de masquer le travail de configuration provider et des clés API derrière un seul workflow d’éditeur. Le post dit que les utilisateurs peuvent choisir un modèle, commencer à coder, suivre leur consommation et gérer leurs coûts depuis un seul endroit, tandis que GLM-5.2 a été lancé avec un prix réduit de 50 %. Cela en fait une note concrète d’intégration éditeur et d’accès pour les équipes qui veulent un chemin d’essai GLM à faible friction.

Type: Integration | Date: 2026-07-20

---
<a id="case-256"></a>
### Case 256: [Évaluation design parallèle dans Pen.dev](https://x.com/tomkrcha/status/2079253857834828230) (by [@tomkrcha](https://x.com/tomkrcha))

**Utilisez ce cas pour comparer GLM-5.2 à des pairs orientés design dans un même workflow local, car tomkrcha dit que pen.dev peut exécuter GLM 5.2 en parallèle avec ChatGPT, Claude, Cursor Composer, OpenCode Go, Kimi, Grok, Gemini, Qwen et d’autres sur des tâches de design directement dans une codebase desktop.**

tomkrcha présente pen.dev comme une surface d’agent design en temps réel plutôt qu’une démo mono-modèle. Le post dit que les builders peuvent se connecter avec leurs abonnements ChatGPT ou Claude existants, brancher Cursor Composer, GitHub Copilot, OpenCode Go, Grok 4.5, Kimi, MiniMax, GLM 5.2, Gemini et Qwen, puis les mélanger et les comparer sur des tâches de design frontend dans le même projet local. Cela en fait une surface d’évaluation side-by-side concrète pour GLM-5.2 sur un travail UI dense, pas un énième screenshot de benchmark isolé.
...
<a id="case-239"></a>
### Case 239: [Fenêtre gratuite d’API GLM chez TokenRouter](https://x.com/meliasiih/status/2078180641468985564) (by [@meliasiih](https://x.com/meliasiih))

**Utilisez ce cas pour saisir une route d’API GLM-5.2 gratuite à court terme, car meliasiih dit que TokenRouter offre un accès gratuit jusqu’au 25 juillet 2026 avec une inscription simple, un flux de clé API et une base URL publiée.**

meliasiih détaille un chemin d’accès pratique : s’inscrire avec un email ou X, vérifier le compte, générer une clé API, choisir GLM-5.2 et pointer les clients vers `https://api.tokenrouter.com/v1`. La source donne aussi directement l’échéance opérationnelle en précisant que le programme d’accès gratuit court jusqu’au 25 juillet 2026, ce qui en fait une note d’accès hébergé bornée dans le temps sur laquelle les équipes peuvent agir immédiatement.

Type: Tutorial | Date: 2026-07-17

---
<a id="case-238"></a>
### Case 238: [Gateway GLM Wafer sur Agentuity](https://x.com/wafer_ai/status/2078186124258984374) (by [@wafer_ai](https://x.com/wafer_ai))

**Utilisez ce cas pour ajouter GLM-5.2 à une stack gateway sur Agentuity, car wafer_ai dit que sa route serverless sert désormais GLM 5.2 sur Agentuity à environ 100 à 250 tok/s avec 1M de contexte sur les paliers standard et Fast.**

wafer_ai dit que la route serverless GLM 5.2 la plus rapide du marché est désormais disponible via Agentuity AI Gateway et que Wafer y sert à la fois les variantes standard et Fast. Le post donne un profil de déploiement exploitable plutôt qu’une note de disponibilité vague : environ 100 à 250 tokens par seconde et une fenêtre de contexte de 1M tokens sur les deux paliers.
...
---
<a id="case-254"></a>
### Case 254: [Serving sparse 860K sur 8x GB10](https://x.com/light_foundry/status/2079158726658060652) (by [@light_foundry](https://x.com/light_foundry))

**Utilisez ce cas pour benchmarker le serving local long contexte de GLM-5.2 sur des clusters de classe GB10, car light_foundry dit qu’une pile 8x GB10 avec indexeur sparse et poids Int4-Int8Mix a atteint 1,101 tok/s de prefill à 4K de profondeur, servi jusqu’à 860K tokens et retrouvé une aiguille à 845K de contexte.**

light_foundry décrit la migration d’un cluster 8x GB10 vers une base unifiée avec l’indexeur sparse `b12x` pour GLM-5.2 Int4-Int8Mix avec fp8 sparse-MLA KV et MTP. Le post rapporte un prefill passé de 880 à 1,101 tok/s à 4K de profondeur, des mesures de depth-prefill de 1,052 / 940 / 838 tok/s à 77K / 231K / 395K, un decode mono-flux médian de 42.1 tok/s et un pool KV de 903K tokens qui a permis un serving à 860K de longueur maximale tout en gardant 10.9 tok/s de decode vers 845K de profondeur. Il signale aussi deux pièges opérationnels concrets : une compilation de cache Inductor à froid qui ressemble à un blocage de mémoire partagée, et une fragmentation mémoire GPU qui réduit le KV disponible après des redémarrages répétés.

Type: Evaluation | Date: 2026-07-20

---
<a id="case-253"></a>
### Case 253: [Compromis local hybride en 3/4/8 bits](https://x.com/0xSero/status/2079184434188685668) (by [@0xSero](https://x.com/0xSero))

**Utilisez ce cas pour dimensionner une route locale GLM-5.2 fortement quantifiée avant de revenir à BF16, car 0xSero dit qu’un build hybride 3/4/8 bits a quand même atteint 70.8 pour cent sur Terminal-Bench 2.1, 88.89 pour cent sur GPQA Diamond, environ 82 tok/s sur 4x RTX 6000 et autour de 0.22 cent par tâche.**

Le post rassemble à la fois des chiffres d’évaluation et de serving pour un setup quantifié GLM-5.2-Hybrid : 70.8 pour cent sur Terminal-Bench 2.1 avec des runs jusqu’à 77.8 pour cent, 88.89 pour cent sur GPQA Diamond, environ 1,200 tok/s en prefill, environ 82 tok/s en decode sur 4x RTX 6000, environ 340K de contexte et autour de 0.22 cent par tâche. Il affirme aussi que l’écart avec BF16 n’est que de 3 à 8 points sur Terminal-Bench et de 0.15 sur GPQA, ce qui en fait une référence de compromis concrète pour les équipes qui doivent décider combien de qualité elles peuvent céder pour réduire le coût et la mémoire du serving local de GLM.
...
**Utilisez ce cas pour juger quand un cluster GLM-5.2 posé au bureau peut remplacer des dépenses d'API hébergée, car thelichhh dit que huit DGX Spark ont été reliés comme une seule machine avec 1TB de mémoire unifiée, ont chargé GLM-5.2 sur tous les nœuds et ont exécuté le modèle à 400K de contexte.**

thelichhh dit que le montage a commencé avec huit DGX Spark éparpillés sur un bureau, puis qu'une correction réseau environ 54 minutes plus tard les a transformés en un seul cluster. Le post relie explicitement ce build à l'économie d'inférence: un ami qui pilote environ 52 mille dollars de factures d'API par mois a vu le cluster GLM 5.2 à 400K de contexte et a décidé d'acheter du hardware. C'est donc une référence concrète de scale-up pour les équipes qui arbitrent entre complexité de cluster et dépense d'API récurrente.

Type: Démo | Date: 2026-07-18

---
<a id="case-245"></a>
### Case 245: [Pulsar avec CPU expert lane](https://x.com/Giannisanii/status/2078430789075656904) (by [@Giannisanii](https://x.com/Giannisanii))

**Utilisez ce cas pour tester une pile locale GLM-5.2 à faible VRAM, car Giannisanii dit que la CPU expert lane de Pulsar a fait passer le throughput de GLM-5.2 744B de 1.6 tok/s à 2.8 tok/s sur deux GeForce 16GB, un NVMe et 32GB de RAM.**

Giannisanii dit que la nouvelle CPU expert lane calcule les experts là où ils se trouvent déjà dans la RAM hôte, au lieu de renvoyer ces octets via PCIe vers le GPU. Le post affirme qu'un noyau AVX2 a atteint 42 GB/s sur un 9900X, au-dessus du coût bus de 28.7 GB/s, et rapporte que GLM-5.2 744B est passé de 1.6 tok/s à 2.8 tok/s sur la même machine sans coût de qualité. C'est une note concrète d'optimisation de déploiement local, pas un simple éloge générique des open models.

Type: Démo | Date: 2026-07-18

---
<a id="case-244"></a>
### Case 244: [Fenêtre GLM 3K sur Peezy Go](https://x.com/SerPepeXBT/status/2078503202346156194) (by [@SerPepeXBT](https://x.com/SerPepeXBT))

**Utilisez ce cas pour comparer un accès GLM-5.2 flat-rate par plafond de requests plutôt que par calcul de tokens, car SerPepeXBT dit que le plan Peezy Go réinitialise désormais les limits, donne jusqu'à 3,000 requests GLM 5.2 toutes les 5 heures, garde le prix à 10 dollars par mois et baisse le premier mois à 5 dollars.**

SerPepeXBT dit que le provider a remis les limits de tout le monde à zéro après une panne et a republié l'allocation GLM 5.2 à 3,000 requests par 5 heures. Le même post ajoute que le plan Go reste à 10 dollars par mois, que le premier mois passe à 5 dollars et que l'application Mac native est désormais sortie. Cela en fait une note concrète d'accès hébergé et de pricing pour les équipes qui évaluent des surfaces de coding par abonnement plutôt qu'une simple facturation d'API.

Type: Intégration | Date: 2026-07-18

<a id="case-242"></a>
### Case 242: [Reçu ZenMux de 249M tokens](https://x.com/AstridWiegner/status/2077917345893511266) (by [@AstridWiegner](https://x.com/AstridWiegner))

**Utilisez ce cas pour vérifier l’économie réelle de GLM-5.2 à partir de reçus plutôt que de tarifs catalogue, car AstridWiegner dit qu’un ZenMux Token Receipt montrait plus de 249M tokens traités, un coût initial de 105.81 dollars et un total final de 0 dollar.**

AstridWiegner dit qu’un ZenMux Token Receipt a enregistré plus de 249 millions de tokens GLM 5.2 et affiché un coût initial de 105.81 dollars avant que le total final ne tombe à 0.00 dollar. Le post présente ce reçu comme une surface de comparaison plus utile que les seuls scores de benchmark, car il relie la qualité de sortie et la taille du workload à la quantité réelle de travail qu’un budget fixe peut acheter.

Type: Evaluation | Date: 2026-07-17

---
<a id="case-241"></a>
### Case 241: [Essai GLM 300M sur Zro Pro](https://x.com/AndarkFomo/status/2078092015368368574) (by [@AndarkFomo](https://x.com/AndarkFomo))

**Utilisez ce cas pour tester un travail agentique GLM-5.2 hébergé en privé avec un budget serré, car AndarkFomo dit qu’une promo Zro Pro peut débloquer environ 300M de tokens GLM-5.2 pour environ 1 dollar avec un accès compatible OpenAI, une infra UE et un positionnement zero-retention.**

AndarkFomo dit que la promo Product Hunt `PRODUCTHUNT` offre au premier groupe de 100 utilisateurs un mois gratuit de Zro Pro, normalement facturé 20 dollars par mois, tandis que certains passages en caisse ne déclenchent qu’une retenue bancaire de 1 dollar. Le post dit que le plan expose une inférence privée de modèles open-weight pour coding agents via une API compatible OpenAI sur une infrastructure UE sans entraînement sur les prompts, et ajoute des réserves pratiques : le chiffre de 300M tokens correspond à un usage attendu plutôt qu’à un plafond dur permanent, les places sont limitées et GLM-5.2 tourne avec 350K de contexte sur cette route.

Type: Tutorial | Date: 2026-07-17

---
<a id="case-240"></a>
### Case 240: [Serving desktop 256K sur DGX Station](https://x.com/TheAhmadOsman/status/2078247891370442867) (by [@TheAhmadOsman](https://x.com/TheAhmadOsman))

**Utilisez ce cas pour dimensionner un déploiement GLM-5.2 de classe desktop, car TheAhmadOsman dit que GLM 5.2 NVFP4 a tourné avec 256K de contexte sur une DGX Station avec environ 3,000 tok/s en prefill et 32 tok/s en decode.**

TheAhmadOsman dit que ce run utilisait GLM 5.2 NVFP4 avec un cache KV en FP8 sur une DGX Station et atteignait environ 3,000 tokens par seconde pendant le prefill et 32 tokens par seconde pendant le decode à 256K de longueur de contexte. Le post signale aussi clairement le compromis : pas encore de requêtes concurrentes, mais un débit mono-machine solide pour une configuration locale à long contexte.
...
**Utilisez ce cas pour justifier un sandboxing plus strict autour des coding agents GLM-5.2, car weezerOSINT dit qu'une exécution ZCode avec un seul prompt a exécuté du code du dépôt avec une RCE complète par défaut et que le problème a été signalé puis corrigé en version 3.3.6.**

weezerOSINT dit que le test était simple: ouvrir un dépôt préparé dans ZCode, donner une seule instruction à l'agent, puis observer l'outil exécuter du code dans le repo avec les réglages par défaut. Le même post dit que GLM 5.2 est le modèle utilisé pour trouver le problème, avertit que la variante UNC pouvait récupérer des identifiants Windows à l'ouverture, et indique que le bug a été divulgué puis corrigé dans ZCode 3.3.6. Cela en fait un signal de sécurité concret, pas un avertissement hypothétique.

Type: Limite | Date: 2026-07-18
