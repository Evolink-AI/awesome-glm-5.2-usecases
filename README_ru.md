...
| Кейс | Фокус | Тип |
|---|---|---|
| [Case 207: Stable Fluids Browser Benchmark](#case-207) | Используйте этот кейс, чтобы сравнить GLM-5.2 на браузерных physics-build’ах с сильной алгоритмической составляющей: AlicanKiraz0 запустил HTML-benchmark Stable Fluids и дал GLM 5.2 Max 88 из 100. | Оценка |
| [Case 199: Epoch Open-Weight Index Lead](#case-199) | Используйте этот кейс, чтобы расположить GLM-5.2 на долгосрочной capability curve: Epoch AI оценивает его в 152 балла на своем Capabilities Index и называет лучшей open-weight моделью в своем наборе оценок. | Бенчмарк |
| [Case 196: Databricks Internal Harness Eval](#case-196) | Используйте этот кейс, чтобы benchmark-ить GLM-5.2 на большой private engineering codebase: Databricks пишет, что внутренняя оценка по работе более чем 3.000 engineers показала очень сильный результат GLM 5.2, а один только выбор harness может снизить стоимость примерно в 2x. | Оценка |
| [Case 190: NatureBench Open-Weight Runner-Up](#case-190) | Используйте этот кейс, чтобы benchmark-ить GLM-5.2 на scientific-agent задачах: NatureBench пишет, что GLM-5.2 дебютировал вторым в общем зачете и стал лидером среди open weights на 90 задачах в шести научных доменах. | Бенчмарк |
| [Case 189: Terminal-Bench 45-Task Cost Tradeoff](#case-189) | Используйте этот кейс, чтобы сравнить GLM-5.2 и GPT-5.5 в одном и том же agent harness: в 45 задачах Terminal-Bench GLM-5.2 взял 25 побед против 29 у GPT-5.5, но стоил примерно на 40% дешевле с учетом prompt caching. | Оценка |
| [Case 188: Harvey LAB-AA Legal-Agent Tie](#case-188) | Используйте этот кейс, чтобы benchmark-ить GLM-5.2 на реальной работе legal agent: Harvey LAB-AA показывает для GLM-5.2 Max 7,5% all-pass, то есть ничью с Claude Opus 4.8 на 120 private tasks в 24 юридических областях. | Бенчмарк |
| [Case 184: AutomationBench-AA Open-Weights Lead](#case-184) | Используйте этот кейс, чтобы сравнить GLM-5.2 не только на code benchmark, но и на SaaS automation с business rules: Artificial Analysis сообщает 27,8% для GLM-5.2 Max и называет его лидером среди open weights на AutomationBench-AA. | Оценка |
...
| Кейс | Фокус | Тип |
|---|---|---|
| [Case 194: CuTeDSL Kernel Skill Open Source](#case-194) | Используйте этот кейс, чтобы изучить GLM-5.2 внутри переиспользуемого skill для kernel-debugging: автор открыл исходники CuTeDSL Hermes skill и пишет, что GLM-5.2 оказался особенно cost-efficient при отладке и написании kernels. | Руководство |
...
| Кейс | Фокус | Тип |
|---|---|---|
| [Case 202: Command Code Space Shooter Feature Win](#case-202) | Используйте этот кейс, чтобы сравнить GLM-5.2 на one-shot interactive UI build: Command Code прогнал один и тот же retro space-shooter prompt через Fable 5, GPT 5.5, GLM 5.2 и DeepSeek V4 Pro и поставил GLM на первое место по features. | Оценка |
| [Case 200: ZCode Nintendo DS Emulator](#case-200) | Используйте этот кейс, чтобы посмотреть на долгий локальный coding build: автор запустил GLM-5.2 в ZCode на 4x RTX 6000 с целью с нуля построить Nintendo DS emulator на C++. | Демонстрация |
| [Case 192: Command Code Flappy Bird UX Split](#case-192) | Используйте этот кейс, чтобы сравнить GLM-5.2 на легких design-game задачах: автор прогнал один и тот же Flappy Bird prompt через Command Code и пришел к выводу, что Fable 5 не был заметно лучше по UX, хотя стоил почти в девять раз дороже GLM-5.2. | Оценка |
...
| Кейс | Фокус | Тип |
|---|---|---|
| [Case 208: Open Molecular Viewer Agent Stack](#case-208) | Используйте этот кейс, чтобы подключить GLM-5.2 к открытому workflow научной инспекции: MaziyarPanahi связал GLM-5.2 через Hugging Face Inference Providers с Qwen3-VL на llama.cpp, Mol* и PydanticAI, чтобы одним prompt’ом отрендерить и прокомментировать структуру EGFR плюс erlotinib. | Интеграция |
| [Case 204: Perplexity Advisor WANDR Cost Baseline](#case-204) | Используйте этот кейс, чтобы оценить экономику GLM-5.2 внутри routed computer-use harness: Perplexity говорит, что его связка GLM 5.2 plus advisor дает 2.1x на WANDR против 6.1x у Opus и в среднем стоит почти вдвое дешевле. | Оценка |
| [Case 203: Coworker Open Artifacts Routing](#case-203) | Используйте этот кейс, чтобы встроить GLM-5.2 в enterprise artifact workflow: Coworker говорит, что Open Artifacts может создавать docs, decks, PDF, spreadsheets, dashboards и apps, а optimized router снижает расход токенов примерно в 5 раз и при этом оставляет US-hosted GLM 5.2. | Интеграция |
| [Case 201: DotCode Context Upload Workflow](#case-201) | Используйте этот кейс, чтобы дать GLM-5.2 более богатый project context внутри private coding sandbox: DotCode добавил поддержку GLM 5.2 вместе с загрузкой screenshot, image, CSV, PDF, source file и zip в один и тот же filesystem-and-terminal workflow. | Интеграция |
| [Case 198: Dahl 100M Free GLM Endpoint](#case-198) | Используйте этот кейс, чтобы попробовать GLM-5.2 через OpenAI-compatible маршрут без карты: Dahl Inference дает 100M бесплатных токенов для GLM 5.2 FP8 и показывает, как создать ключ и вызвать `/v1/chat/completions`. | Руководство |
| [Case 195: NVIDIA Free Endpoint GLM Setup](#case-195) | Используйте этот кейс, чтобы попробовать GLM-5.2 в coding tools без self-hosting: источник описывает бесплатный NVIDIA endpoint flow, который заводит API-ключи GLM-5.2 в Claude Code, Cursor или Cline. | Руководство |
| [Case 193: 0G TeeML Private Inference Route](#case-193) | Используйте этот кейс, чтобы провести GLM-5.2 через privacy-first provider path: 0G пишет, что GLM 5.2 теперь работает в TeeML, где prompt’ы запечатаны внутри TEE enclave, а цена ниже официального маршрута. | Интеграция |
| [Case 185: DuckDB Flock Open-SQL PR](#case-185) | Используйте этот кейс, чтобы принести GLM-5.2 в полностью открытый локальный SQL analysis: lhoestq пишет, что PR для duckdb плюс flock уже включает GLM-5.2 внутри 100% open-source data stack. | Интеграция |
...
| Кейс | Фокус | Тип |
|---|---|---|
| [Case 209: Colibri 25GB RAM Sparse Streaming](#case-209) | Используйте этот кейс, чтобы понять новую нижнюю границу локальных экспериментов с GLM-5.2: techNmak подробно объясняет, как Colibrì запускает 744B MoE примерно на 25 ГБ RAM, стримя experts с NVMe, хотя самый маленький setup дает лишь около 0,05–0,1 tok/s. | Демо |
| [Case 206: SGLang NVFP4 Production Throughput](#case-206) | Используйте этот кейс, чтобы оценить production SGLang serving для GLM-5.2 NVFP4: официальный релиз SGLang v0.5.15 говорит о более чем 500 tok/s на пользователя на 8x B300 и 450 на 4x GB300 при batch size 1. | Оценка |
| [Case 191: Hermes-Built LiteLLM Local Lab](#case-191) | Используйте этот кейс, чтобы поднять local inference lab с GLM-5.2 в роли coding agent: источник говорит, что Hermes Agent плюс GLM-5.2 настроили LiteLLM, Postgres, Prometheus и Grafana вокруг M3 Ultra setup. | Интеграция |
| [Case 187: Dual M5 Max Offline Droneship Sim](#case-187) | Используйте этот кейс, чтобы оценить, на что способен полностью offline GLM-5.2 agent на Apple Silicon: XavierLocalAI пишет, что 753B setup на двух 128GB M5 Max пишет симулятор посадки на droneship со скоростью 26 tok/s. | Демонстрация |
| [Case 186: 5x DGX Spark Production Harness](#case-186) | Используйте этот кейс, чтобы понять, хватает ли пятиузловой DGX Spark конфигурации для production-работы с GLM-5.2: thatcofffeeguy сообщает примерно 13,9 tok/s в single-stream на 400K context и 19,9 tok/s суммарно по трем lane в live harness. | Демонстрация |
...
| Кейс | Фокус | Тип |
|---|---|---|
| [Case 205: Static HTML Rewrite Executor Misses](#case-205) | Используйте этот кейс, чтобы не отдавать GLM-5.2 полный executor-контроль на 1:1 legacy rewrite: при большой миграции со static HTML на React и Vite через OpenCode Go и Cline потерялось слишком много деталей, и автор стал больше доверять GLM как planner, а не как executor. | Ограничение |
| [Case 197: Composio 47-Task Agent Gaps](#case-197) | Используйте этот кейс, чтобы понять, где GLM-5.2 все еще ломается на реальной SaaS-agent работе: Composio подключила его к 17 инструментам на 47 задачах и получила 45 успешных проходов, а сбои возникли на проверке полноты и в нечетком SLA judgment. | Оценка |
...
---
<a id="case-207"></a>
### Case 207: [Stable Fluids Browser Benchmark](https://x.com/AlicanKiraz0/status/2075639232169705781) (автор [@AlicanKiraz0](https://x.com/AlicanKiraz0))
...
Тип: Evaluation | Дата: 2026-07-10

<a id="case-199"></a>
### Case 199: [Epoch Open-Weight Index Lead](https://x.com/EpochAIResearch/status/2074894535558300103) (автор [@EpochAIResearch](https://x.com/EpochAIResearch))
...
Тип: Бенчмарк | Дата: 2026-07-08

---
<a id="case-196"></a>
### Case 196: [Databricks Internal Harness Eval](https://x.com/alighodsi/status/2074996561306955958) (автор [@alighodsi](https://x.com/alighodsi))
...
Тип: Оценка | Дата: 2026-07-08

---
<a id="case-190"></a>
### Case 190: [NatureBench Open-Weight Runner-Up](https://x.com/OkhayIea/status/2074498200262889837) (автор [@OkhayIea](https://x.com/OkhayIea))
...
Тип: Бенчмарк | Дата: 2026-07-07

---
<a id="case-189"></a>
### Case 189: [Terminal-Bench 45-Task Cost Tradeoff](https://x.com/Aiswarya_Sankar/status/2074554064856314219) (автор [@Aiswarya_Sankar](https://x.com/Aiswarya_Sankar))
...
Тип: Оценка | Дата: 2026-07-07

---
<a id="case-188"></a>
### Case 188: [Harvey LAB-AA Legal-Agent Tie](https://x.com/ArtificialAnlys/status/2074541975186165887) (автор [@ArtificialAnlys](https://x.com/ArtificialAnlys))
...
---
<a id="case-184"></a>
### Case 184: [AutomationBench-AA Open-Weights Lead](https://x.com/ArtificialAnlys/status/2074194764510208230) (автор [@ArtificialAnlys](https://x.com/ArtificialAnlys))
...
<a id="case-194"></a>
### Case 194: [CuTeDSL Kernel Skill Open Source](https://x.com/SubhoGhosh02/status/2074466050557739469) (автор [@SubhoGhosh02](https://x.com/SubhoGhosh02))
...
Тип: Руководство | Дата: 2026-07-07
...
<a id="case-202"></a>
### Case 202: [Command Code Space Shooter Feature Win](https://x.com/CommandCodeAI/status/2075264795817972107) (автор [@CommandCodeAI](https://x.com/CommandCodeAI))
...
Тип: Оценка | Дата: 2026-07-09

---

<a id="case-200"></a>
### Case 200: [ZCode Nintendo DS Emulator](https://x.com/0xSero/status/2074870153267818638) (автор [@0xSero](https://x.com/0xSero))
...
Тип: Демонстрация | Дата: 2026-07-08

---
<a id="case-192"></a>
### Case 192: [Command Code Flappy Bird UX Split](https://x.com/MrAhmadAwais/status/2074536879308026031) (автор [@MrAhmadAwais](https://x.com/MrAhmadAwais))
...
<a id="case-208"></a>
### Case 208: [Open Molecular Viewer Agent Stack](https://x.com/MaziyarPanahi/status/2075913552854933869) (автор [@MaziyarPanahi](https://x.com/MaziyarPanahi))
...
Тип: Интеграция | Дата: 2026-07-11

---
<a id="case-204"></a>
### Case 204: [Perplexity Advisor WANDR Cost Baseline](https://x.com/perplexity_ai/status/2075229779716973030) (автор [@perplexity_ai](https://x.com/perplexity_ai))
...
<a id="case-203"></a>
### Case 203: [Coworker Open Artifacts Routing](https://x.com/coworkerapp/status/2075233366266310905) (автор [@coworkerapp](https://x.com/coworkerapp))
...
Тип: Интеграция | Дата: 2026-07-09

---

<a id="case-201"></a>
### Case 201: [DotCode Context Upload Workflow](https://x.com/stagedhappen/status/2074775356867789241) (автор [@stagedhappen](https://x.com/stagedhappen))
...
Тип: Интеграция | Дата: 2026-07-08

---
<a id="case-209"></a>
### Case 209: [Colibri 25GB RAM Sparse Streaming](https://x.com/techNmak/status/2075872446197158361) (автор [@techNmak](https://x.com/techNmak))
...
Тип: Демо | Дата: 2026-07-11

---
<a id="case-206"></a>
### Case 206: [SGLang NVFP4 Production Throughput](https://x.com/sgl_project/status/2075721488456654861) (автор [@sgl_project](https://x.com/sgl_project))
...
---
<a id="case-198"></a>
### Case 198: [Dahl 100M Free GLM Endpoint](https://x.com/pengsonal/status/2074908680106180669) (автор [@pengsonal](https://x.com/pengsonal))
...
Тип: Руководство | Дата: 2026-07-08

---
<a id="case-195"></a>
### Case 195: [NVIDIA Free Endpoint GLM Setup](https://x.com/undefinedKi/status/2074491917333655948) (автор [@undefinedKi](https://x.com/undefinedKi))
...
---
<a id="case-193"></a>
### Case 193: [0G TeeML Private Inference Route](https://x.com/0G_labs/status/2074496704959676682) (автор [@0G_labs](https://x.com/0G_labs))
...
Тип: Интеграция | Дата: 2026-07-07

---
<a id="case-185"></a>
### Case 185: [DuckDB Flock Open-SQL PR](https://x.com/lhoestq/status/2074143736624275629) (автор [@lhoestq](https://x.com/lhoestq))
...
Тип: Интеграция | Дата: 2026-07-06
...
---
<a id="case-176"></a>
### Case 176: [NVIDIA NIM OpenCode Thinking Setup](https://x.com/Dracoshowumore/status/2073384581256929717) (от [@Dracoshowumore](https://x.com/Dracoshowumore), [@lhoestq](https://x.com/lhoestq), [@XavierLocalAI](https://x.com/XavierLocalAI))
...
<a id="case-191"></a>
### Case 191: [Hermes-Built LiteLLM Local Lab](https://x.com/ivanfioravanti/status/2074609005272375329) (автор [@ivanfioravanti](https://x.com/ivanfioravanti))
...
---
<a id="case-187"></a>
### Case 187: [Dual M5 Max Offline Droneship Sim](https://x.com/XavierLocalAI/status/2073938465121833452) (автор [@XavierLocalAI](https://x.com/XavierLocalAI))
...
Тип: Демонстрация | Дата: 2026-07-06

---
<a id="case-186"></a>
### Case 186: [5x DGX Spark Production Harness](https://x.com/thatcofffeeguy/status/2074245620207058981) (автор [@thatcofffeeguy](https://x.com/thatcofffeeguy))
...
<a id="case-205"></a>
### Case 205: [Static HTML Rewrite Executor Misses](https://x.com/petruknisme/status/2075092910182387759) (автор [@petruknisme](https://x.com/petruknisme))
...
Тип: Ограничение | Дата: 2026-07-09

---

<a id="case-197"></a>
### Case 197: [Composio 47-Task Agent Gaps](https://x.com/composio/status/2074908761970393265) (автор [@composio](https://x.com/composio))
